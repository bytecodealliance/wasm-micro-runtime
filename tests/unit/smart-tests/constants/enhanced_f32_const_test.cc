 ...
 *
 * This class provides comprehensive testing infrastructure for the f32.const
 * WebAssembly opcode, ensuring proper constant loading functionality across
 * different execution modes (interpreter and AOT). Tests validate that
 * f32.const correctly pushes immediate 32-bit IEEE 754 floating-point values
 * onto the execution stack without consuming any stack operands. Includes
 * comprehensive validation of special IEEE 754 values including NaN, infinity,
 * zero, subnormals, and boundary conditions.
 */
class F32ConstTest : public testing::TestWithParam<RunningMode>
{
  protected:
     ...
     *
     * Initializes WAMR runtime with appropriate configuration for testing
     * f32.const operations. Configures memory allocation, execution mode, and
     * loads the f32.const test module. Ensures proper runtime state before
     ...
     *
     * Performs proper cleanup of WASM module instances, modules, and runtime
     * resources. Ensures no memory leaks or resource conflicts between test
     * cases. Maintains clean test environment for subsequent test execution.
     */
    ...
    float call_const_func(const char *func_name)
    {
        ...
        EXPECT_NE(func_inst, nullptr)
            << "Failed to lookup function: " << func_name;

        wasm_val_t results[1];
        wasm_val_t arguments[1]; // f32.const doesn't need arguments but
                                 // declaring for consistency

        bool success = wasm_runtime_call_wasm_a(exec_env, func_inst, 1, results,
                                                0, arguments);
        EXPECT_TRUE(success)
            << "Failed to call function: " << func_name
    ...
    /**
     * Execute multiple f32.const test function and return the loaded constant
     * values
     *
     * @param func_name Name of the WASM function to execute (must return
     * multiple f32 values)
     * @param result_count Number of f32 values to expect
     * @param results Array to store the returned f32 constant values
     */
    void call_multi_const_func(const char *func_name, uint32_t result_count,
                               float *results)
    {
        wasm_function_inst_t func_inst =
            wasm_runtime_lookup_function(module_inst, func_name);
        ...
        bool success = wasm_runtime_call_wasm_a(
            exec_env, func_inst, result_count, wasm_results, 0, arguments);
        ASSERT_TRUE(success)
            << "Failed to call function: " << func_name
            << ", exception: " << wasm_runtime_get_exception(module_inst);
 ...
 * @brief Validates f32.const produces correct values for typical floating-point
 * inputs
 * @details Tests fundamental constant loading operation with positive,
 * negative, and zero values. Verifies that f32.const correctly loads standard
 * IEEE 754 f32 values onto stack.
 * @test_category Main - Basic functionality validation
 * @coverage_target
 * core/iwasm/interpreter/wasm_interp_classic.c:f32_const_operation
 ...
 * @details Tests NaN, positive/negative infinity, positive/negative zero to
 * ensure f32.const correctly handles all special IEEE 754 single-precision
 * values.
 * @test_category Edge - Special IEEE 754 value validation
 * @coverage_target
 * core/iwasm/interpreter/wasm_interp_classic.c:f32_const_operation
 * @input_conditions NaN, +/-infinity, +/-zero IEEE 754 values
 * @expected_behavior Exact preservation of special values with correct IEEE 754
 * semantics
 * @validation_method Special value checks and bitwise comparison for signed
 * zeros
 */
TEST_P(F32ConstTest, SpecialValues_PreservesIEEE754)
{
    ...
    ASSERT_TRUE(std::isinf(call_const_func("get_pos_inf"))
                && call_const_func("get_pos_inf") > 0)
        << "f32.const failed to preserve positive infinity";

    // Test negative infinity
    ASSERT_TRUE(std::isinf(call_const_func("get_neg_inf"))
                && call_const_func("get_neg_inf") < 0)
 ...
 *          stack management and ensure no interference between constant
 * operations.
 ...
 * @validation_method Verification of multiple returned values in correct
 * sequence
 */
 ...
 * @details Tests using f32.const values in f32.add operations to ensure
 * constants are properly loaded and available for arithmetic operations.
...
/**
 * @test f32_const_EdgeCaseValues_PreservesAccuracy
 * @brief Validates f32.const handles edge case floating-point values
 * @details Tests f32.const with values at floating-point representation
 * boundaries.
 * @test_category Edge - Boundary value handling
 * @coverage_target
 * core/iwasm/interpreter/wasm_interp_classic.c:f32_const_operation
 * @input_conditions Values at floating-point representation boundaries
 * @expected_behavior Correct handling of edge case values without precision
 * loss
 * @validation_method Precision verification for boundary values
 */

// Parameterized test instantiation for both interpreter and AOT modes
INSTANTIATE_TEST_SUITE_P(
    RunningModeTest, F32ConstTest, testing::Values(Mode_Interp, Mode_LLVM_JIT),
