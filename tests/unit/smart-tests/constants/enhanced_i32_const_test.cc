 ...
 *
 * This class provides comprehensive testing infrastructure for the i32.const
 ...
 * i32.const correctly pushes immediate 32-bit signed integer values onto the
 * execution stack without consuming any stack operands.
 */
class I32ConstTest : public testing::TestWithParam<RunningMode>
{
  protected:
     ...
     * i32.const operations. Configures memory allocation, execution mode, and
     * loads the i32.const test module. Ensures proper runtime state before
    ...
    /**
     * Test invalid function call scenario for failure path testing
     */
    bool call_invalid_func(const char *func_name)
    {
        wasm_function_inst_t func_inst =
            wasm_runtime_lookup_function(module_inst, func_name);
        if (!func_inst) {
        ...
        uint32_t argv[1] = { 0 };
        return wasm_runtime_call_wasm(exec_env, func_inst, 0, argv);
    }
};
 ...
 * negative, and zero values. Verifies that i32.const correctly pushes immediate
 * values onto the execution stack.
 * @test_category Main - Basic functionality validation
 * @coverage_target
 * core/iwasm/interpreter/wasm_interp_classic.c:i32_const_operation
 * @input_conditions Standard integer values: 1, -1, 42, -42, 100, -100, 0
 * @expected_behavior Returns exact constant values: 1, -1, 42, -42, 100, -100,
 * 0
 * @validation_method Direct comparison of WASM function result with expected
 * constant values
 */
 ...
 * @brief Validates i32.const handles boundary values correctly (INT32_MIN,
 * INT32_MAX)
 * @details Tests extreme boundary conditions with minimum and maximum 32-bit
 * signed integer values. Verifies that boundary values maintain exact bit
 * representation without overflow.
 * @test_category Corner - Boundary condition validation
 * @coverage_target
 * core/iwasm/interpreter/wasm_interp_classic.c:i32_const_boundary_handling
 * @input_conditions INT32_MIN (-2147483648), INT32_MAX (2147483647), adjacent
 * values
 * @expected_behavior Returns exact boundary values with proper sign handling
 * @validation_method Direct comparison with INT32_MIN/MAX constants and bit
 * pattern validation
 */
 ...
 * @brief Validates error conditions at module and runtime level are handled
 * properly
 * @details Tests error handling for invalid modules and runtime initialization
 * failures. Verifies that error conditions are properly reported without
 * causing crashes.
 ...
 * @expected_behavior Proper error handling without crashes, informative error
 * messages
 * @validation_method Verification that invalid operations return null/failure
 * as expected
 */
TEST_P(I32ConstTest, ModuleLevelErrors_HandleGracefully)
{
    // Test invalid module loading (simulate by trying to load with invalid
    // buffer)
    char error_buffer[128];
    uint8_t invalid_wasm[] = {
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
    }; // Completely invalid WASM data

    wasm_module_t invalid_module = wasm_runtime_load(
        invalid_wasm, sizeof(invalid_wasm), error_buffer, sizeof(error_buffer));
...
/**
 * @test MinimalResources_HandlesEdgeCase
 * @brief Tests module instantiation at resource boundary conditions
 * @details Validates instantiation behavior with minimal but sufficient
 * resources.
 */

// Parameterized test instantiation for both interpreter and AOT modes
INSTANTIATE_TEST_SUITE_P(
    RunningModeTest, I32ConstTest, testing::Values(Mode_Interp, Mode_LLVM_JIT),
