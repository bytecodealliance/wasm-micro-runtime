 ...
 */
 ...
 /**
  * Test Function 2: wasm_call_indirect() - Success path
 ...
 TEST_F(FunctionInvocationTest, WasmCallIndirect_Success_ExecutesCorrectly)
 {
     ...
     uint32_t wasm_argv[2];
     wasm_argv[0] = 1;  // table index 1 (points to $mul_func)
     wasm_argv[1] = 7;  // parameter value
 
     bool success = wasm_runtime_call_wasm(exec_env, func, 2, wasm_argv);
     ASSERT_TRUE(success);
 
     // $mul_func multiplies by 2: 7 * 2 = 14
     ASSERT_EQ(wasm_argv[0], 14);
 }
 ...
 TEST_F(FunctionInvocationTest, ExecuteMalloc_Failure_HandlesLargeAllocation)
 {
     ...
     if (success) {
         // If it succeeds, the result should be 0 (allocation failure)
