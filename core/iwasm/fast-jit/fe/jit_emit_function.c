                             ...
                             uint32 tbl_idx)
{
    ...
    JitFrame *jit_frame = cc->jit_frame;
    // get module instance
    JitReg module_inst = get_module_inst_reg(jit_frame);
    // get table_instance
    JitReg table_inst = get_table_inst_reg(jit_frame, tbl_idx);
    // get table_cur_size
    JitReg tbl_size = get_table_cur_size_reg(jit_frame, tbl_idx);
    ...
    // sget func_idx and check if -1 or out of bounder

    // get func_idx
    JitReg elem_idx_long = jit_cc_new_reg_I64(cc);
    ...
    if (!jit_emit_exception(cc, JIT_EXCE_INVALID_FUNCTION_INDEX, JIT_OP_BEQ,
                            cc->cmp_reg, NULL))
        goto fail;
