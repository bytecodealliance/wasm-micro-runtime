...
#define REG_AOT_SYM(symbol) { "__"#symbol, (void*)__aot_##symbol }
                        ...
                        char *error_buf, uint32 error_buf_size)
{
    ...
    wasm_assert(comp_ctx->exec_engine);
    ...
    if (comp_data->start_func_index != (uint32)-1) {
        wasm_assert(comp_data->start_func_index >= module->import_func_count
                        ...
                        char *error_buf, uint32 error_buf_size)
{
    ...
    if (!comp_data) {
        ...
        wasm_assert(aot_last_error != NULL);
