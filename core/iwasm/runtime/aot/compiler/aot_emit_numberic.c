...
static char *bit_cnt_llvm_intrinsic[] = { "llvm.ctlz.32",
                                          "llvm.ctlz.64",
                                          "llvm.cttz.32",
                                          "llvm.cttz.64",
                                          "llvm.ctpop.32",
                ...
                IntArithmetic arith_op, bool is_i32, uint8 **p_frame_ip)
{
    ...
    wasm_assert(arith_op == INT_DIV_S
                || arith_op == INT_DIV_U
                || arith_op == INT_REM_S
                || arith_op == INT_REM_U);
...
compile_int_shl(AOTCompContext *comp_ctx,
                ...
                bool is_i32)
{
    LLVMValueRef shift_count_mask, res;

    SHIFT_COUNT_MASK;

    /* Build shl */
    LLVM_BUILD_OP(Shl, left, shift_count_mask, res, "shl", NULL);
                  ...
                  bool is_i32)
{
    ...
    /* Build shl */
    LLVM_BUILD_OP(AShr, left, shift_count_mask, res, "shr_s", NULL);
                  ...
                  bool is_i32)
{
    ...
    /* Build shl */
    LLVM_BUILD_OP(LShr, left, shift_count_mask, res, "shr_u", NULL);
...
compile_int_rot(AOTCompContext *comp_ctx,
                 ...
                 bool is_i32)
{
    LLVMValueRef shift_count_mask, bits_minus_shift_count, res, tmp_l, tmp_r;
    ...
    if (is_rotl) {
        ...
        LLVM_BUILD_OP(Shl, left, shift_count_mask, tmp_l, "tmp_l", NULL);
        LLVM_BUILD_OP(LShr, left, bits_minus_shift_count, tmp_r, "tmp_r", NULL);
    }
    else {
        /* left>>count | left<<(BITS-count) */
        LLVM_BUILD_OP(LShr, left, shift_count_mask, tmp_l, "tmp_l", NULL);
