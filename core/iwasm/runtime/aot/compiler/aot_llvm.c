                  ...
                  uint32 func_index)
{
    ...
    /* aot module instance as first parameter */
    param_types[j++] = comp_ctx->aot_inst_type;
                   ...
                   LLVMTypeRef int8_ptr_type, uint32 func_index)
{
    ...
    /* Load ext memory base address */
    offset = I32_CONST(offsetof(AOTModuleInstance, ext_mem_data.ptr));
    if (!(func_ctx->ext_mem_base_addr =
                LLVMBuildInBoundsGEP(comp_ctx->builder, func_ctx->aot_inst,
                                     &offset, 1, "ext_mem_base_addr_offset"))) {
      aot_set_last_error("llvm build in bounds gep failed");
      return false;
    }
    if (!(func_ctx->ext_mem_base_addr =
                LLVMBuildBitCast(comp_ctx->builder, func_ctx->ext_mem_base_addr,
                                 int8_ptr_type, "ext_mem_base_addr_tmp"))) {
        aot_set_last_error("llvm build bit cast failed");
        return false;
    }
    if (!(func_ctx->ext_mem_base_addr =
                LLVMBuildLoad(comp_ctx->builder, func_ctx->ext_mem_base_addr,
                              "ext_mem_base_addr"))) {
        aot_set_last_error("llvm build load failed");
        return false;
    }

    /* Load ext_mem base offset */
    offset = I32_CONST(offsetof(AOTModuleInstance, ext_mem_base_offset));
    if (!(func_ctx->ext_mem_base_offset =
                LLVMBuildInBoundsGEP(comp_ctx->builder, func_ctx->aot_inst,
                                     &offset, 1, "ext_mem_base_offset_offset"))) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
    if (!(func_ctx->ext_mem_base_offset =
                LLVMBuildBitCast(comp_ctx->builder, func_ctx->ext_mem_base_offset,
                                 INT32_PTR_TYPE, "ext_mem_base_offset_tmp"))) {
        aot_set_last_error("llvm build bit cast failed");
        return false;
    }
    if (!(func_ctx->ext_mem_base_offset =
                LLVMBuildLoad(comp_ctx->builder, func_ctx->ext_mem_base_offset,
                              "ext_mem_base_offset"))) {
        aot_set_last_error("llvm build load failed");
        return false;
    }

    /* Load ext memory data size */
    offset = I32_CONST(offsetof(AOTModuleInstance, ext_mem_data_size));
    if (!(func_ctx->ext_mem_data_size =
                LLVMBuildInBoundsGEP(comp_ctx->builder, func_ctx->aot_inst,
                                     &offset, 1, "ext_mem_data_size_offset"))) {
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
    if (!(func_ctx->ext_mem_data_size =
                LLVMBuildBitCast(comp_ctx->builder, func_ctx->ext_mem_data_size,
                                 INT32_PTR_TYPE, "ext_mem_data_size_tmp"))) {
        aot_set_last_error("llvm build bit cast failed");
        return false;
    }
    if (!(func_ctx->ext_mem_data_size =
                LLVMBuildLoad(comp_ctx->builder, func_ctx->ext_mem_data_size,
                              "ext_mem_data_size"))) {
        aot_set_last_error("llvm build load failed");
        return false;
    }
                        ...
                        AOTFunc *func, uint32 func_index)
{
    ...
    j = 0;
    func_ctx->aot_inst = LLVMGetParam(func_ctx->func, j++);
    ...
    /* Create base addr, end addr, data size of mem, heap and ext_mem */
...
static bool
check_target_triple(const char *triple)
{
    uint32 i;
    char *arch;
    bool support_eb;
    ...
    for (i = 0; i < sizeof(valid_archs) / sizeof(ArchItem); i++) {
        ...
        if (!strncmp(triple, arch, strlen(arch))
            && ((support_eb && (!strcmp(triple + strlen(arch), "eb")
                                || !strcmp(triple + strlen(arch), "")))
                || (!support_eb && !strcmp(triple + strlen(arch), "")))) {
            return true;
        }
    }
    return false;
                        ...
                        aot_comp_option_t option)
{
    ...
    char *triple, *triple_norm, *cpu, *features, buf[128];
    ...
    if (option->is_jit_mode) {
        ...
        comp_ctx->is_fast_call_conv = true;
#endif
    }
    else {
        /* Create LLVM target machine */
        triple = option->target_triple;
        ...
        if (triple) {
            ...
            if (!strcmp(triple, "arm"))
                triple = "armv4";
            else if (!strcmp(triple, "armeb"))
                triple = "armv4eb";
            else if (!strcmp(triple, "thumb"))
                triple = "thumbv4t";
            else if (!strcmp(triple, "thumbeb"))
                triple = "thumbv4teb";
        }

        if (triple && !check_target_triple(triple)) {
            if (!strcmp(triple, "help"))
                print_supported_targets();
            else
        ...
        if (triple) {
            /* Add default -<vendor>-<sys>-<abi> */
            const char *vendor_sys_abi = "-pc-linux-gnu";
            bh_assert(strlen(triple) + strlen(vendor_sys_abi) < sizeof(triple_buf));
            memcpy(triple_buf, triple, strlen(triple));
            memcpy(triple_buf + strlen(triple), vendor_sys_abi, strlen(vendor_sys_abi));
            triple = triple_buf;
        }
    ...
    /* Add LLVM data type of AOTContext struct */
...
aot_value_stack_pop(AOTValueStack *stack)
{
    ...
    wasm_assert(stack->value_list_end);
...
aot_block_stack_pop(AOTBlockStack *stack)
{
    ...
    wasm_assert(stack->block_list_end);
