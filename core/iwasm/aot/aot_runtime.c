                       ...
                       char *error_buf, uint32 error_buf_size)
{
    ...
    if (global_index >= module->import_global_count
        || module->import_globals->common.is_mutable) {
    ...
    if (global_index < module->import_global_count
        && module->import_globals[global_index].common.is_mutable) {
        set_error_buf(error_buf, error_buf_size,
                      "constant expression required");
        return false;
    }
                   ...
                   char *error_buf, uint32 error_buf_size)
{
    ...
    for (i = 0; i < module->import_global_count; i++, import_global++) {
        ...
        init_global_data(p, import_global->common.type,
        ...
        switch (flag) {
            case INIT_EXPR_TYPE_GET_GLOBAL:
            {
                ...
                if (init_expr->u.global_index < module->import_global_count) {
                    ...
                    uint32 global_idx =
                        init_expr->u.global_index - module->import_global_count;
                    init_global_data(p, global->common.type,
            ...
            default:
            {
                init_global_data(p, global->common.type, &init_expr->u);
                break;
            }
        }
        p += global->size;
...
aot_global_traverse_gc_rootset(AOTModuleInstance *module_inst, void *heap)
{
    ...
    for (i = 0; i < module->import_global_count; i++, import_global++) {
        if (wasm_is_type_reftype(import_global->common.type)) {
    ...
    for (i = 0; i < module->global_count; i++, global++) {
        if (wasm_is_type_reftype(global->common.type)) {
            gc_obj = GET_REF_FROM_ADDR((uint32 *)global_data);
            if (wasm_obj_is_created_from_heap(gc_obj)) {
                if (0 != mem_allocator_add_root((mem_allocator_t)heap, gc_obj))
                    return false;
