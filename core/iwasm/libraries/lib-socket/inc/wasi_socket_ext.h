...
static void ipv4_addr_to_wasi_addr(uint32_t addr_num, uint16_t port,
                                   __wasi_addr_t *out) {
    out->kind = IPv4;
...
static __wasi_errno_t sockaddr_to_wasi_addr(const struct sockaddr *sock_addr,
                                            socklen_t addrlen,
...
static __wasi_errno_t sock_addr_remote(__wasi_fd_t fd,
