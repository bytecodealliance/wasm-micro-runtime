...
static void
cluster_lock_thread_list(WASMCluster *cluster)
{
                            ...
                            void *user_data)
{
    ...
    while (node) {
        ...
        visitor(node, user_data);
        cluster_lock_thread_list(cluster);
...
thread_manager_start_routine(void *arg)
{
    ...
    if (exec_env->suspend_flags.flags & THREAD_RET_VALUE_FLAG)
...
{
    return exec_env->current_status.running_status == WASM_THREAD_RUNNING
           || exec_env->current_status.running_status == WASM_THREAD_STEP;
}
...
{
    exec_env->current_status.running_status = WASM_THREAD_STOP;
...
{
    exec_env->current_status.running_status = WASM_THREAD_EXIT;
...
wasm_cluster_exit_thread(WASMExecEnv *exec_env, void *retval)
{
    ...
    if (exec_env->jmpbuf_stack_top) {
        ...
        exec_env->suspend_flags.flags |= THREAD_RET_VALUE_FLAG;
...
#endif
    exec_env->suspend_flags.flags |= THREAD_TERMINATE_FLAG;
...
{
    /* TODO: implement wasm_cluster_suspend_thread */
}
...
{
    /* TODO: implement wasm_cluster_resume_thread */
}
