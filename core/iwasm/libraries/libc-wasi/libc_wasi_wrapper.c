                 ...
                 const char *new_path, uint32 new_path_len)
{
    ...
    rename_fd(old_fd, old_path, new_fd, new_path);
#endif
...
{
#if WASM_ENABLE_CHECKPOINT_RESTORE!=0
    LOG_STRACE("wasi_sock_close exec_env=%d, fd=%d \n", exec_env, fd);
    remove_fd(fd);
#endif
...
#if WASM_ENABLE_CHECKPOINT_RESTORE!=0
    LOG_STRACE( "wasi_sock_set_ip_multicast_ttl exec_env=%d, fd=%d, ttl_s=%d \n",
               exec_env, fd, ttl_s);
#endif
...
#if WASM_ENABLE_CHECKPOINT_RESTORE!=0
    LOG_STRACE("wasi_sock_set_ipv6_only exec_env=%d, fd=%d, is_enabled=%d \n",exec_env, fd, is_enabled);
#endif
