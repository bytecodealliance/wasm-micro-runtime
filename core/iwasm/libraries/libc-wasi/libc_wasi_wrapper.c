                    ...
                    wasi_prestat_app_t *prestat_app)
{
    ...
    LOG_STRACE("wasi_fd_prestat_get exec_env=%d, fd=%d \n", exec_env, fd);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                         ...
                         uint32 path_len)
{
    ...
    LOG_STRACE("wasi_fd_prestat_dir_name exec_env=%d, fd=%d, path=%s, path_len=%d \n", exec_env, fd, path, path_len);
...
wasi_fd_close(wasm_exec_env_t exec_env, wasi_fd_t fd)
{
    ...
    LOG_STRACE("wasi_fd_close exec_env=%d, fd=%d \n", exec_env, fd);
...
wasi_fd_datasync(wasm_exec_env_t exec_env, wasi_fd_t fd)
{
    ...
    LOG_STRACE("wasi_fd_datasync exec_env=%d, fd=%d \n", exec_env, fd);
              ...
              uint32 iovs_len, wasi_filesize_t offset, uint32 *nread_app)
{
    ...
    *nread_app = (uint32)nread;
    ...
    /* success */
    err = 0;
    LOG_STRACE("wasi_fd_pread exec_env=%d, fd=%d \n", exec_env, fd);
               ...
               wasi_filesize_t offset, uint32 *nwritten_app)
{
    ...
    *nwritten_app = (uint32)nwritten;
    ...
    /* success */
    err = 0;
    LOG_STRACE("wasi_fd_pwrite exec_env=%d, fd=%d, iovec_app=%d, iovs_len=%d, offset=%d, nwritten_app=%d \n", exec_env, fd, iovec_app, iovs_len, offset, nwritten_app);
                   ...
                   wasi_fdstat_t *fdstat_app)
{
    ...
    LOG_STRACE("wasi_fd_fdstat_get exec_env=%d, fd=%d \n", exec_env, fd);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                         ...
                         wasi_fdflags_t flags)
{
    ...
    LOG_STRACE("wasi_fd_fdstat_set_flags exec_env=%d, fd=%d, flags=%d \n", exec_env, fd, flags);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                          ...
                          wasi_rights_t fs_rights_inheriting)
{
    ...
    LOG_STRACE("wasi_fd_fdstat_set_rights exec_env=%d, fd=%d, fs_rights_base=%d, fs_rights_inheriting=%d \n", exec_env, fd, fs_rights_base, fs_rights_inheriting);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
...
wasi_fd_sync(wasm_exec_env_t exec_env, wasi_fd_t fd)
{
    ...
    LOG_STRACE("wasi_fd_sync exec_env=%d, fd=%d \n", exec_env, fd);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
              ...
              uint32 *nwritten_app)
{
    ...
    if (!validate_native_addr(nwritten_app, (uint32)sizeof(uint32))
        ...
        return (wasi_errno_t)-1;
// for STrace
    LOG_STRACE("wasi_fd_sync exec_env=%d, fd=%d, iovec_app=%d, iovs_len=%d \n", exec_env, fd, iovec_app, iovs_len);
    total_size = sizeof(wasi_ciovec_t) * (uint64)iovs_len;
    if (total_size >= UINT32_MAX
        || !(ciovec_begin = wasm_runtime_malloc((uint32)total_size)))
        return (wasi_errno_t)-1;
               ...
               uint32 new_path_len)
{
    ...
    LOG_STRACE("wasi_path_link exec_env=%d, old_fd=%d, old_flags=%d, old_path=%s, old_path_len=%d, new_fd=%d, new_path=%s, new_path_len=%d \n", exec_env, old_fd, old_flags, old_path, old_path_len, new_fd, new_path, new_path_len);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
               ...
               wasi_fd_t *fd_app)
{
    ...
    LOG_STRACE("wasi_path_open exec_env=%d, dirfd=%d, dirflags=%d, path=%s, path_len=%d, oflags=%d, fs_rights_base=%d, fs_rights_inheriting=%d, fs_flags=%d, fd_app=%d \n", exec_env, dirfd, dirflags, path, path_len, oflags, fs_rights_base, fs_rights_inheriting, fs_flags, fd_app);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                ...
                uint32 buf_len, wasi_dircookie_t cookie, uint32 *bufused_app)
{
    ...
    LOG_STRACE("wasi_fd_readdir exec_env=%d, fd=%d, buf=%d, buf_len=%d, cookie=%d, bufused_app=%d \n", exec_env, fd, buf, buf_len, cookie, bufused_app);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                   ...
                   uint32 *bufused_app)
{
    ...
    LOG_STRACE("wasi_path_readlink exec_env=%d, fd=%d, path=%s, path_len=%d, buf=%d, buf_len=%d, bufused_app=%d \n", exec_env, fd, path, path_len, buf, buf_len, bufused_app);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                 ...
                 const char *new_path, uint32 new_path_len)
{
    ...
    LOG_STRACE("wasi_path_rename exec_env=%d, old_fd=%d, old_path=%s, old_path_len=%d, new_fd=%d, new_path=%s, new_path_len=%d \n", exec_env, old_fd, old_path, old_path_len, new_fd, new_path, new_path_len);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                     ...
                     wasi_filestat_t *filestat)
{
    ...
    LOG_STRACE("wasi_fd_filestat_get exec_env=%d, fd=%d, filestat=%d \n", exec_env, fd, filestat);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                           ...
                           wasi_fstflags_t fstflags)
{
    ...
    LOG_STRACE("wasi_fd_filestat_set_times exec_env=%d, fd=%d, st_atim=%d, st_mtim=%d, fstflags=%d \n", exec_env, fd, st_atim, st_mtim, fstflags);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                          ...
                          wasi_filesize_t st_size)
{
    ...
    LOG_STRACE("wasi_fd_filestat_set_size exec_env=%d, fd=%d, st_size=%d \n", exec_env, fd, st_size);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                             ...
                             wasi_timestamp_t st_mtim, wasi_fstflags_t fstflags)
{
    ...
    LOG_STRACE("exec_env=%d, fd=%d, flags=%d, path=%s, path_len=%d, st_atim=%d, st_mtim=%d, fstflags=%d \n", exec_env, fd, flags, path, path_len, st_atim, st_mtim, fstflags);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                  ...
                  uint32 new_path_len)
{
    ...
    LOG_STRACE("wasi_path_symlink exec_env=%d, old_path=%s, old_path_len=%d, fd=%d, new_path=%s, new_path_len=%d \n", exec_env, old_path, old_path_len, fd, new_path, new_path_len);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                      ...
                      uint32 path_len)
{
    ...
    LOG_STRACE("wasi_path_unlink_file exec_env=%d, fd=%d, path=%s, path_len=%d \n", exec_env, fd, path, path_len);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                           ...
                           const char *path, uint32 path_len)
{
    ...
    LOG_STRACE("wasi_path_remove_directory exec_env=%d, fd=%d, path=%s, path_len=%d \n", exec_env, fd, path, path_len);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                 ...
                 wasi_event_t *out, uint32 nsubscriptions, uint32 *nevents_app)
{
    ...
    LOG_STRACE("wasi_poll_oneoff exec_env=%d, in=%d, out=%d, nsubscriptions=%d, nevents_app=%d \n", exec_env, in, out, nsubscriptions, nevents_app);
    if (!wasi_ctx)
        return (wasi_errno_t)-1;
                 ...
                 wasi_fd_t *fd_new)
{
    ...
    LOG_STRACE("wasi_sock_accept exec_env=%d, fd=%d, flags=%d, fd_new=%d \n", exec_env, fd, flags, fd_new);
    if (!wasi_ctx)
                     ...
                     __wasi_addr_t *addr)
{
    ...
    LOG_STRACE("wasi_sock_addr_local exec_env=%d, fd=%d, addr=%d \n", exec_env, fd, addr);
    if (!wasi_ctx)
        return __WASI_EACCES;
                      ...
                      __wasi_addr_t *addr)
{
    ...
    LOG_STRACE("wasi_sock_addr_remote exec_env=%d, fd=%d, addr=%d \n", exec_env, fd, addr);
    if (!wasi_ctx)
        return __WASI_EACCES;
                       ...
                       __wasi_size_t *max_info_size)
{
    ...
    LOG_STRACE("wasi_sock_addr_resolve exec_env=%d, host=%s, service=%s, hints=%d, addr_info=%d, addr_info_size=%d, max_info_size=%d \n", exec_env, host, service, hints, addr_info, addr_info_size, max_info_size);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_bind(wasm_exec_env_t exec_env, wasi_fd_t fd, wasi_addr_t *addr)
{
    ...
    LOG_STRACE("wasi_sock_bind exec_env=%d, fd=%d, addr=%d \n", exec_env, fd, addr);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_connect(wasm_exec_env_t exec_env, wasi_fd_t fd, wasi_addr_t *addr)
{
    ...
    LOG_STRACE("wasi_sock_connect exec_env=%d, fd=%d, addr=%d \n", exec_env, fd, addr);
    if (!wasi_ctx)
        return __WASI_EACCES;
                        ...
                        bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_broadcast exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                         ...
                         bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_keep_alive exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                     ...
                     int *linger_s)
{
    ...
    LOG_STRACE("wasi_sock_get_linger exec_env=%d, fd=%d, is_enabled=%d, linger_s=%d \n", exec_env, fd, is_enabled, linger_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
                            ...
                            size_t *size)
{
    ...
    LOG_STRACE("wasi_sock_get_recv_buf_size exec_env=%d, fd=%d, size=%d \n", exec_env, fd, size);
    if (!wasi_ctx)
        return __WASI_EACCES;
                           ...
                           uint64_t *timeout_us)
{
    ...
    LOG_STRACE("wasi_sock_get_recv_timeout exec_env=%d, fd=%d, timeout_us=%d \n", exec_env, fd, timeout_us);
    if (!wasi_ctx)
        return __WASI_EACCES;
                         ...
                         bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_reuse_addr exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                         ...
                         bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_reuse_port exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                            ...
                            size_t *size)
{
    ...
    LOG_STRACE("wasi_sock_get_send_buf_size exec_env=%d, fd=%d, size=%d \n", exec_env, fd, size);
    if (!wasi_ctx)
        return __WASI_EACCES;
                           ...
                           uint64_t *timeout_us)
{
    ...
    LOG_STRACE("wasi_sock_get_send_timeout exec_env=%d, fd=%d, timeout_us=%d \n", exec_env, fd, timeout_us);
    if (!wasi_ctx)
        return __WASI_EACCES;
                                   ...
                                   bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_tcp_fastopen_connect exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                           ...
                           bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_tcp_no_delay exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                            ...
                            bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_tcp_quick_ack exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                            ...
                            uint32_t *time_s)
{
    ...
    LOG_STRACE("wasi_sock_get_tcp_keep_idle exec_env=%d, fd=%d, time_s=%d \n", exec_env, fd, time_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
                             ...
                             uint32_t *time_s)
{
    ...
    LOG_STRACE("wasi_sock_get_tcp_keep_intvl exec_env=%d, fd=%d, time_s=%d \n", exec_env, fd, time_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
                                ...
                                bool ipv6, bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_ip_multicast_loop exec_env=%d, fd=%d, ipv6=%d, is_enabled=%d \n", exec_env, fd, ipv6, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_get_ip_ttl(wasm_exec_env_t exec_env, wasi_fd_t fd, uint8_t *ttl_s)
{
    ...
    LOG_STRACE("wasi_sock_get_ip_ttl exec_env=%d, fd=%d, ttl_s=%d \n", exec_env, fd, ttl_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
                               ...
                               uint8_t *ttl_s)
{
    ...
    LOG_STRACE("wasi_sock_get_ip_multicast_ttl exec_env=%d, fd=%d, ttl_s=%d \n", exec_env, fd, ttl_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
                        ...
                        bool *is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_get_ipv6_only exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_listen(wasm_exec_env_t exec_env, wasi_fd_t fd, uint32 backlog)
{
    ...
    LOG_STRACE("wasi_sock_listen exec_env=%d, fd=%d, backlog=%d \n", exec_env, fd, backlog);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_set_broadcast(wasm_exec_env_t exec_env, wasi_fd_t fd, bool is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_set_broadcast exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                         ...
                         bool is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_set_keep_alive exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                     ...
                     int linger_s)
{
    ...
    LOG_STRACE("wasi_sock_set_linger exec_env=%d, fd=%d, is_enabled=%d, linger_s=%d \n", exec_env, fd, is_enabled, linger_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_set_recv_buf_size(wasm_exec_env_t exec_env, wasi_fd_t fd, size_t size)
{
    ...
    LOG_STRACE("wasi_sock_set_recv_buf_size exec_env=%d, fd=%d, size=%d \n", exec_env, fd, size);
    if (!wasi_ctx)
        return __WASI_EACCES;
                           ...
                           uint64_t timeout_us)
{
    ...
    LOG_STRACE("wasi_sock_set_recv_timeout exec_env=%d, fd=%d, timeout_us=%d \n", exec_env, fd, timeout_us);
    if (!wasi_ctx)
        return __WASI_EACCES;
                         ...
                         bool is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_set_reuse_addr exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                         ...
                         bool is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_set_reuse_port exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_set_send_buf_size(wasm_exec_env_t exec_env, wasi_fd_t fd, size_t size)
{
    ...
    LOG_STRACE("wasi_sock_set_send_buf_size exec_env=%d, fd=%d, size=%d \n", exec_env, fd, size);
    if (!wasi_ctx)
        return __WASI_EACCES;
                           ...
                           uint64_t timeout_us)
{
    ...
    LOG_STRACE("wasi_sock_set_send_timeout exec_env=%d, fd=%d, timeout_us=%d \n", exec_env, fd, timeout_us);
    if (!wasi_ctx)
        return __WASI_EACCES;
                                   ...
                                   bool is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_set_tcp_fastopen_connect exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                           ...
                           bool is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_set_tcp_no_delay exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                            ...
                            bool is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_set_tcp_quick_ack exec_env=%d, fd=%d, is_enabled=%d \n", exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                            ...
                            uint32_t time_s)
{
    ...
    LOG_STRACE("wasi_sock_set_tcp_keep_idle exec_env=%d, fd=%d, time_s=%d \n", exec_env, fd, time_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
                             ...
                             uint32_t time_s)
{
    ...
    LOG_STRACE("wasi_sock_set_tcp_keep_intvl exec_env=%d, fd=%d, time_s=%d \n", exec_env, fd, time_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
                                ...
                                bool ipv6, bool is_enabled)
{
    ...
    LOG_STRACE("wasi_sock_set_ip_multicast_loop exec_env=%d, fd=%d, ipv6=%d, is_enabled=%d \n", exec_env, fd, ipv6, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
                                ...
                                uint32_t imr_interface)
{
    ...
    LOG_STRACE("wasi_sock_set_ip_add_membership exec_env=%d, fd=%d, imr_multiaddr=%d, imr_interface=%d \n", exec_env, fd, imr_multiaddr, imr_interface);
    if (!wasi_ctx)
        return __WASI_EACCES;
                                 ...
                                 uint32_t imr_interface)
{
    ...
    LOG_STRACE("wasi_sock_set_ip_drop_membership exec_env=%d, fd=%d, imr_multiaddr=%d, imr_interface=%d \n", exec_env, fd, imr_multiaddr, imr_interface);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_set_ip_ttl(wasm_exec_env_t exec_env, wasi_fd_t fd, uint8_t ttl_s)
{
    ...
    LOG_STRACE("wasi_sock_set_ip_ttl exec_env=%d, fd=%d, ttl_s=%d \n", exec_env, fd, ttl_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
                               ...
                               uint8_t ttl_s)
{
    ...
    LOG_STRACE(
        "wasi_sock_set_ip_multicast_ttl exec_env=%d, fd=%d, ttl_s=%d \n",
        exec_env, fd, ttl_s);
    if (!wasi_ctx)
        return __WASI_EACCES;
...
wasi_sock_set_ipv6_only(wasm_exec_env_t exec_env, wasi_fd_t fd, bool is_enabled)
{
    ...
    LOG_STRACE(
        "wasi_sock_set_ipv6_only exec_env=%d, fd=%d, is_enabled=%d \n",
        exec_env, fd, is_enabled);
    if (!wasi_ctx)
        return __WASI_EACCES;
               ...
               wasi_roflags_t *ro_flags)
{
    ...
    LOG_STRACE("wasi_sock_recv exec_env=%d, sock=%d, ri_data=%d, ri_data_len=%d, ri_flags=%d, ro_data_len=%d, ro_flags=%d \n", exec_env, sock, ri_data, ri_data_len, ri_flags, ro_data_len, ro_flags);
               ...
               wasi_siflags_t si_flags, uint32 *so_data_len)
{
    ...
    LOG_STRACE("wasi_sock_send exec_env=%d, sock=%d, si_data=%d, si_data_len=%d, si_flags=%d, so_data_len=%d \n", exec_env, sock, si_data, si_data_len, si_flags, so_data_len);
    if (!wasi_ctx) {
        return __WASI_EINVAL;
    }
...
wasi_sock_shutdown(wasm_exec_env_t exec_env, wasi_fd_t sock, wasi_sdflags_t how)
{
    ...
    LOG_STRACE("wasi_sock_shutdown exec_env=%d, sock=%d, how=%d \n", exec_env, sock, how);
    if (!wasi_ctx)
        return __WASI_EINVAL;
