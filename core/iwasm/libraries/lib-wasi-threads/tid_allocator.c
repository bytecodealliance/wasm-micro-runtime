...
tid_allocator_init(TidAllocator *tid_allocator)
{
    ...
    for (int64 i = tid_allocator->pos - 1; i >= 0; i--)
        tid_allocator->ids[i] = (uint32)(TID_MIN + (tid_allocator->pos - 1 - i));
...
tid_allocator_get_tid(TidAllocator *tid_allocator)
{
    if (tid_allocator->pos == 0) { // Resize stack and push new thread ids
        ...
        int32 *tmp = wasm_runtime_realloc(tid_allocator->ids, (uint32)realloc_size);
        if (tmp == NULL) {
        ...
        for (int64 i = tid_allocator->pos - 1; i >= 0; i--)
            tid_allocator->ids[i] = (uint32)(TID_MIN + (tid_allocator->size - 1 - i));
    }
