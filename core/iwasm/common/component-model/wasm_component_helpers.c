...
// Set an error message in the provided buffer, supporting printf-style formatting
...
bool
parse_result_list(const uint8_t **payload, const uint8_t *end, WASMComponentResultList **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;

    // Allocate memory for the result list structure
    *out = wasm_runtime_malloc(sizeof(WASMComponentResultList));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for result list");
        return false;
    }
    ...
    switch (tag) {
        case WASM_COMP_RESULT_LIST_WITH_TYPE: {
            ...
            if (!(*out)->results) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for result value type");
                return false;
            }
            memset((*out)->results, 0, sizeof(WASMComponentValueType));
            
            if (!parse_valtype(&p, end, (*out)->results, error_buf, error_buf_size)) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to parse result");
                return false;
            }
            break;
        }
        case WASM_COMP_RESULT_LIST_EMPTY: {
            ...
            if (terminator != 0x00) {
                set_error_buf_ex(error_buf, error_buf_size, "Invalid empty resultlist terminator: 0x%02x", terminator);
                return false;
            }
            break;
        }
        default: {
            set_error_buf_ex(error_buf, error_buf_size, "Invalid result list tag: 0x%02x", tag);
            return false;
        }
    }
...
bool
parse_sort(const uint8_t **payload, const uint8_t *end, WASMComponentSort *out, char *error_buf, uint32_t error_buf_size, bool is_core)
{
    ...
    if (!is_core) {
        ...
        switch (out->sort) {
            ...
            case WASM_COMP_SORT_INSTANCE:    // 0x05
                break;
            default:
                set_error_buf_ex(error_buf, error_buf_size, "Invalid sort value: 0x%02x", out->sort);
                *payload = p;
                return false;
        }
    }
    ...
    if (out->sort == 0x00) {
        ...
        if (p >= end) {
            set_error_buf_ex(error_buf, error_buf_size, "Unexpected end of buffer when parsing core_sort");
    ...
    if (out->core_sort != INVALID_VALUE) {
        switch (out->core_sort) {
            ...
            case WASM_COMP_CORE_SORT_INSTANCE:  // 0x12
                break;
            default:
                set_error_buf_ex(error_buf, error_buf_size, "Invalid core sort value: 0x%02x", out->core_sort);
                *payload = p;
                return false;
        }
    }
    // For any other value of s->sort, the s->core_sort field remains 0 and is ignored.
...
bool
parse_sort_idx(const uint8_t **payload, const uint8_t *end, WASMComponentSortIdx *out, char *error_buf, uint32_t error_buf_size, bool is_core)
{
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    ...
    if (!out->sort) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for sort");
        return false;
    }
    
    // Parse the sort first
    bool status = parse_sort(payload, end, out->sort, error_buf, error_buf_size, is_core);
    if (!status) {
    ...
    uint64_t idx_leb = 0;
    if (!read_leb((uint8_t **)payload, end, 32, false, &idx_leb, error_buf, error_buf_size)) {
        wasm_runtime_free(out->sort);
        return false;
    }
...
bool
parse_extern_desc(const uint8_t **payload, const uint8_t *end, WASMComponentExternDesc *out, char *error_buf, uint32_t error_buf_size){
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    switch (out->type) {
        ...
        case WASM_COMP_EXTERN_VALUE: {
            ...
            out->extern_desc.value.value_bound = wasm_runtime_malloc(sizeof(WASMComponentValueBound));
            if (!out->extern_desc.value.value_bound) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for value_bound");
                *payload = p; // Update pointer even on error
                return false;
            }
            ...
            switch (value_bound_tag) {
                case WASM_COMP_VALUEBOUND_EQ: {
                    ...
                    out->extern_desc.value.value_bound->bound.value_idx = (uint32_t)value_idx;
                    break;
                }
                case WASM_COMP_VALUEBOUND_TYPE: {
                    // 0x01 t:<valtype> => t
                    // Allocate memory for value_type
                    out->extern_desc.value.value_bound->bound.value_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
                    if (!out->extern_desc.value.value_bound->bound.value_type) {
                        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for value_type");
                        wasm_runtime_free(out->extern_desc.value.value_bound);
                        *payload = p; // Update pointer even on error
                        return false;
                    }
                    ...
                    if (!parse_valtype(&p, end, out->extern_desc.value.value_bound->bound.value_type, error_buf, error_buf_size)) {
                        wasm_runtime_free(out->extern_desc.value.value_bound->bound.value_type);
                        wasm_runtime_free(out->extern_desc.value.value_bound);
                        *payload = p; // Update pointer even on error
                        return false;
                    }
                ...
                default: {
                    set_error_buf_ex(error_buf, error_buf_size, "Unknown value bound tag: 0x%02X", value_bound_tag);
                    wasm_runtime_free(out->extern_desc.value.value_bound);
                    *payload = p; // Update pointer even on error
                    return false;
                }
        ...
        case WASM_COMP_EXTERN_TYPE: {
            ...
            if (!out->extern_desc.type.type_bound) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for type_bound");
                *payload = p; // Update pointer even on error
                return false;
            }
            ...
            if (type_bound_tag == WASM_COMP_TYPEBOUND_EQ) {
                ...
                set_error_buf_ex(error_buf, error_buf_size, "Invalid typebound tag: 0x%02x", type_bound_tag);
                wasm_runtime_free(out->extern_desc.type.type_bound);
                *payload = p;
                return false;
            }
        ...
        default: {
            set_error_buf_ex(error_buf, error_buf_size, "Unknown extern_desc type: 0x%02X", out->type);
            *payload = p; // Update pointer even on error
            return false;
        }
    }
...
free_extern_desc(WASMComponentExternDesc *desc)
{
    ...
    switch (desc->type) {
        case WASM_COMP_EXTERN_VALUE: {
            ...
            if (vb) {
                if (vb->tag == WASM_COMP_VALUEBOUND_TYPE && vb->bound.value_type) {
...
bool
parse_core_name(const uint8_t **payload, const uint8_t *end, WASMComponentCoreName **out, char *error_buf, uint32_t error_buf_size)
{
    ...
    if (!read_leb((uint8_t **)&p, end, 32, false, &name_len_leb, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to read core name length");
        *payload = p; // Update pointer even on error
        return false;
    }
    ...
    if (name_len_leb == 0) {
        WASMComponentCoreName *result = wasm_runtime_malloc(sizeof(WASMComponentCoreName));
        if (!result) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for core name structure");
            *payload = p; // Update pointer even on error
            return false;
        }
        
        result->name_len = 0;
        result->name = wasm_runtime_malloc(1); // Allocate 1 byte for null terminator
        if (!result->name) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for core name string");
    ...
    if (p + name_len_leb > end) {
        set_error_buf_ex(error_buf, error_buf_size, "Core name extends beyond buffer");
        *payload = p; // Update pointer even on error
        return false;
    }
...
bool
parse_component_import_name(const uint8_t **payload, const uint8_t *end, WASMComponentImportName *out, char *error_buf, uint32_t error_buf_size)
{
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    ...
    switch (tag) {
        ...
        case WASM_COMP_IMPORTNAME_VERSIONED: {
            ...
            if (!parse_core_name(&p, end, &version, error_buf, error_buf_size)) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to read version name");
                free_core_name(name);
            ...
            bool version_has_at = (version->name_len > 0 && version->name[0] == '@');
            ...
            if (!full) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate versioned name");
                free_core_name(name);
                wasm_runtime_free(name);
            ...
            if (!version_has_at) full[name->name_len] = '@';
            memcpy(full + name->name_len + sep_len, version->name, version->name_len);
            full[full_len] = '\0';
        ...
        default:
            set_error_buf_ex(error_buf, error_buf_size, "Unknown import/export name tag: 0x%02X", tag);
            *payload = p; // Update pointer even on error
            return false;
    }
...
bool
parse_component_export_name(const uint8_t **payload, const uint8_t *end, WASMComponentExportName *out, char *error_buf, uint32_t error_buf_size)
{
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
...
bool parse_valtype(const uint8_t **payload, const uint8_t *end, WASMComponentValueType *out, char *error_buf, uint32_t error_buf_size) {
    ...
    if (is_primitive_type(first_byte)) {
        ...
        uint64_t type_idx = 0;
        if (!read_leb((uint8_t **)payload, end, 32, false, &type_idx, error_buf, error_buf_size)) {
            return false;
        }
...
bool parse_labelvaltype(const uint8_t **payload, const uint8_t *end, WASMComponentLabelValType *out, char *error_buf, uint32_t error_buf_size) {
    ...
    if (!parse_core_name(payload, end, &out->label, error_buf, error_buf_size)) {
        return false;
    }
    ...
    if (!out->value_type) {
        ...
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for label value type");
        return false;
    }
    memset(out->value_type, 0, sizeof(WASMComponentValueType));
    if (!parse_valtype(payload, end, out->value_type, error_buf, error_buf_size)) {
...
bool parse_case(const uint8_t **payload, const uint8_t *end, WASMComponentCaseValType *out, char *error_buf, uint32_t error_buf_size) {
    ...
    if (optional_tag == WASM_COMP_OPTIONAL_TRUE) {
        ...
        out->value_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
        if (!out->value_type) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for case value type");
            free_core_name(out->label);
            return false;
        }
        ...
        set_error_buf_ex(error_buf, error_buf_size, "Malformed binary: invalid optional tag 0x%02x", optional_tag);
        return false;
    }
    ...
    if (ending_byte != WASM_COMP_CASE_END) {
        set_error_buf_ex(error_buf, error_buf_size, "Expected 0x00 at end of case, got 0x%02x", ending_byte);
        if (out->value_type) {
            wasm_runtime_free(out->value_type);
        }
        free_core_name(out->label);
...
bool parse_label_prime(const uint8_t **payload, const uint8_t *end, 
                      WASMComponentCoreName **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if ((uint32_t)(end - p) < len) {
        set_error_buf_ex(error_buf, error_buf_size, "Insufficient bytes for label: expected %u, got %zu", 
                     len, (size_t)(end - p));
        return false;
    }
    ...
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for label");
        return false;
    }
    ...
    if (!(*out)->name) {
        ...
        *out = NULL;
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for label string");
        return false;
    }
...
static bool fill_label_prime(const uint8_t **payload, const uint8_t *end, 
                           WASMComponentCoreName *out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
...
bool parse_label_prime_vector(const uint8_t **payload, const uint8_t *end, WASMComponentCoreName **out_labels, uint32_t *out_count, char *error_buf, uint32_t error_buf_size) {
    ...
    if (count > 0) {
        ...
        *out_labels = wasm_runtime_malloc(sizeof(WASMComponentCoreName) * count);
        if (!*out_labels) {
        ...
        for (uint32_t i = 0; i < count; i++) {
            if (!fill_label_prime(&p, end, &((*out_labels)[i]), error_buf, error_buf_size)) {
...
// Validate a UTF-8 sequence for general correctness (delegates to wasm_check_utf8_str)
...
bool wasm_component_validate_single_utf8_scalar(const uint8_t *bytes, uint32_t len) {
    ...
    if (b0 == 0xED) {
        // 0xED leads into UTF-16 surrogate range; disallow 0x80..0x9F second byte
        if (len != 3) return false;
        const uint8_t b1 = bytes[1], b2 = bytes[2];
        return (b1 >= 0x80 && b1 <= 0x9F) ? false : (b1 >= 0xA0 && b1 <= 0xBF) && (b2 >= 0x80 && b2 <= 0xBF);
    }
    if (b0 >= 0xEE && b0 <= 0xEF) {
        if (len != 3) return false;
        const uint8_t b1 = bytes[1], b2 = bytes[2];
    ...
    if (b0 == 0xF0) {
        ...
        return (b1 >= 0x90 && b1 <= 0xBF) && (b2 >= 0x80 && b2 <= 0xBF) && (b3 >= 0x80 && b3 <= 0xBF);
    }
    if (b0 >= 0xF1 && b0 <= 0xF3) {
        if (len != 4) return false;
        const uint8_t b1 = bytes[1], b2 = bytes[2], b3 = bytes[3];
        return (b1 >= 0x80 && b1 <= 0xBF) && (b2 >= 0x80 && b2 <= 0xBF) && (b3 >= 0x80 && b3 <= 0xBF);
    }
    if (b0 == 0xF4) {
        // Limit to U+10FFFF -> second byte <= 0x8F
        if (len != 4) return false;
        const uint8_t b1 = bytes[1], b2 = bytes[2], b3 = bytes[3];
        return (b1 >= 0x80 && b1 <= 0x8F) && (b2 >= 0x80 && b2 <= 0xBF) && (b3 >= 0x80 && b3 <= 0xBF);
    }
