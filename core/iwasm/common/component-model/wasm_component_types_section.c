...
static void free_component_instance_decl(WASMComponentInstDecl *decl);

// Free helpers for nested component/instance/resource types
static void free_component_types_entry(WASMComponentTypes *type);

// Helper function to parse record types
static bool parse_record_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    ...
    if (!read_leb((uint8_t **)&p, end, 32, false, &count_leb, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse variant type");
        return false;
    }
    ...
    if (count > 0) {
        ...
        if (!(*out)->def_val.record) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for record type");
            return false;
        }
        
        // Allocate the fields array
        (*out)->def_val.record->fields = wasm_runtime_malloc(sizeof(WASMComponentLabelValType) * count);
        if (!(*out)->def_val.record->fields) {
            wasm_runtime_free((*out)->def_val.record);
            (*out)->def_val.record = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for record fields");
            return false;
        }
        ...
        memset((*out)->def_val.record->fields, 0, sizeof(WASMComponentLabelValType) * count);
        
        // Parse each field
        for (uint32_t i = 0; i < count; i++) {
            if (!parse_labelvaltype(&p, end, &(*out)->def_val.record->fields[i], error_buf, error_buf_size)) {
                ...
                wasm_runtime_free((*out)->def_val.record);
                (*out)->def_val.record = NULL;
                set_error_buf_ex(error_buf, error_buf_size, "Failed to parse record type");
                return false;
            }
        }
    } else {
        set_error_buf_ex(error_buf, error_buf_size, "record type must have at least one field");
        return false;
    }
...
static bool parse_variant_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if (count > 0) {
        ...
        (*out)->def_val.variant = wasm_runtime_malloc(sizeof(WASMComponentVariantType));
        if (!(*out)->def_val.variant) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for variant type");
            return false;
        }
        
        // Allocate the cases array
        (*out)->def_val.variant->cases = wasm_runtime_malloc(sizeof(WASMComponentCaseValType) * count);
        if (!(*out)->def_val.variant->cases) {
            wasm_runtime_free((*out)->def_val.variant);
            (*out)->def_val.variant = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for variant cases");
            return false;
        }
        ...
        memset((*out)->def_val.variant->cases, 0, sizeof(WASMComponentCaseValType) * count);
        
        // Parse each case
        for (uint32_t i = 0; i < count; i++) {
            if (!parse_case(&p, end, &(*out)->def_val.variant->cases[i], error_buf, error_buf_size)) {
        ...
        set_error_buf_ex(error_buf, error_buf_size, "variant type must have at least one field");
        return false;
    }
...
static bool parse_tuple_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if (count > 0) {
        ...
        (*out)->def_val.tuple = wasm_runtime_malloc(sizeof(WASMComponentTupleType));
        if (!(*out)->def_val.tuple) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for tuple type");
            return false;
        }
        
        // Allocate the element types array
        (*out)->def_val.tuple->element_types = wasm_runtime_malloc(sizeof(WASMComponentValueType) * count);
        if (!(*out)->def_val.tuple->element_types) {
            wasm_runtime_free((*out)->def_val.tuple);
            (*out)->def_val.tuple = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for tuple element types");
            return false;
        }
        ...
        memset((*out)->def_val.tuple->element_types, 0, sizeof(WASMComponentValueType) * count);
        
        // Parse each element type
        for (uint32_t i = 0; i < count; i++) {
            if (!parse_valtype(&p, end, &(*out)->def_val.tuple->element_types[i], error_buf, error_buf_size)) {
                wasm_runtime_free((*out)->def_val.tuple->element_types);
                wasm_runtime_free((*out)->def_val.tuple);
                (*out)->def_val.tuple = NULL;
                set_error_buf_ex(error_buf, error_buf_size, "Failed to parse tuple element type %u", i);
                return false;
            }
        }
    } else {
        set_error_buf_ex(error_buf, error_buf_size, "tuple type must have at least one field");
        return false;
    }
...
static bool parse_flags_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if (count > 0 && count <= 32) {
        ...
        (*out)->def_val.flag = wasm_runtime_malloc(sizeof(WASMComponentFlagType));
        if (!(*out)->def_val.flag) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for flag type");
            return false;
        }
        
        // Allocate the labels array
        (*out)->def_val.flag->flags = wasm_runtime_malloc(sizeof(WASMComponentCoreName) * count);
        if (!(*out)->def_val.flag->flags) {
            wasm_runtime_free((*out)->def_val.flag);
            (*out)->def_val.flag = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for flag labels");
            return false;
        }
        ...
        memset((*out)->def_val.flag->flags, 0, sizeof(WASMComponentCoreName) * count);
        
        // Parse each label
        for (uint32_t i = 0; i < count; i++) {
            WASMComponentCoreName *temp_ptr = &(*out)->def_val.flag->flags[i];
            if (!parse_label_prime(&p, end, &temp_ptr, error_buf, error_buf_size)) {
                // Clean up on error
                for (uint32_t j = 0; j < i; j++) {
                ...
                wasm_runtime_free((*out)->def_val.flag);
                (*out)->def_val.flag = NULL;
                set_error_buf_ex(error_buf, error_buf_size, "Failed to parse flag label %u", i);
                return false;
            }
        }
    } else {
        set_error_buf_ex(error_buf, error_buf_size, "Flags type must have 0 < count <= 32, got %u", count);
        return false;
    }
...
static bool parse_enum_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if (count > 0) {
        ...
        (*out)->def_val.enum_type = wasm_runtime_malloc(sizeof(WASMComponentEnumType));
        if (!(*out)->def_val.enum_type) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for enum type");
            return false;
        }
        
        // Allocate the labels array
        (*out)->def_val.enum_type->labels = wasm_runtime_malloc(sizeof(WASMComponentCoreName) * count);
        if (!(*out)->def_val.enum_type->labels) {
            wasm_runtime_free((*out)->def_val.enum_type);
            (*out)->def_val.enum_type = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for enum labels");
            return false;
        }
        ...
        // Initialize all labels to zero
        memset((*out)->def_val.enum_type->labels, 0, sizeof(WASMComponentCoreName) * count);
        
        // Parse each label
        for (uint32_t i = 0; i < count; i++) {
            WASMComponentCoreName *temp_ptr = &(*out)->def_val.enum_type->labels[i];
            if (!parse_label_prime(&p, end, &temp_ptr, error_buf, error_buf_size)) {
                // Clean up on error
                for (uint32_t j = 0; j < i; j++) {
                ...
                wasm_runtime_free((*out)->def_val.enum_type);
                (*out)->def_val.enum_type = NULL;
                set_error_buf_ex(error_buf, error_buf_size, "Failed to parse enum label %u", i);
                return false;
            }
        }
    } else {
        set_error_buf_ex(error_buf, error_buf_size, "enum type must have at least one field");
        return false;
    }
...
static bool parse_option_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    (*out)->def_val.option = wasm_runtime_malloc(sizeof(WASMComponentOptionType));
    if (!(*out)->def_val.option) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for option type");
        return false;
    }
    
    // Allocate the element type
    (*out)->def_val.option->element_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
    if (!(*out)->def_val.option->element_type) {
        wasm_runtime_free((*out)->def_val.option);
        (*out)->def_val.option = NULL;
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for option element type");
        return false;
    }
    
    // Initialize element type to zero
    memset((*out)->def_val.option->element_type, 0, sizeof(WASMComponentValueType));

    // Parse option type
    if (!parse_valtype(&p, end, (*out)->def_val.option->element_type, error_buf, error_buf_size)) {
        wasm_runtime_free((*out)->def_val.option->element_type);
        wasm_runtime_free((*out)->def_val.option);
        (*out)->def_val.option = NULL;
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse option type");
        return false;
    }
...
static bool parse_result_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    (*out)->def_val.result = wasm_runtime_malloc(sizeof(WASMComponentResultType));
    if (!(*out)->def_val.result) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for result type");
        return false;
    }
    ...
    if (has_result_type == WASM_COMP_OPTIONAL_TRUE) {
        (*out)->def_val.result->result_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
        if (!(*out)->def_val.result->result_type) {
            wasm_runtime_free((*out)->def_val.result);
            (*out)->def_val.result = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for result result type");
            return false;
        }
        memset((*out)->def_val.result->result_type, 0, sizeof(WASMComponentValueType));
        if (!parse_valtype(&p, end, (*out)->def_val.result->result_type, error_buf, error_buf_size)) {
            wasm_runtime_free((*out)->def_val.result->result_type);
            wasm_runtime_free((*out)->def_val.result);
            (*out)->def_val.result = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to parse result result type");
            return false;
        }
    } else if (has_result_type != WASM_COMP_OPTIONAL_FALSE) {
        set_error_buf_ex(error_buf, error_buf_size, "Malformed binary: invalid optional tag 0x%02x", has_result_type);
        return false;
    }
    ...
    if (has_error_type == WASM_COMP_OPTIONAL_TRUE) {
        (*out)->def_val.result->error_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
        if (!(*out)->def_val.result->error_type) {
            ...
            wasm_runtime_free((*out)->def_val.result);
            (*out)->def_val.result = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for result error type");
            return false;
        }
        memset((*out)->def_val.result->error_type, 0, sizeof(WASMComponentValueType));
        if (!parse_valtype(&p, end, (*out)->def_val.result->error_type, error_buf, error_buf_size)) {
            ...
            wasm_runtime_free((*out)->def_val.result);
            (*out)->def_val.result = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to parse result error type");
            return false;
        }
    } else if (has_error_type != WASM_COMP_OPTIONAL_FALSE) {
        set_error_buf_ex(error_buf, error_buf_size, "Malformed binary: invalid optional tag 0x%02x", has_error_type);
        return false;
    }
...
static bool parse_own_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if (!(*out)->def_val.owned) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for own type");
        return false;
    }

    // Parse own type
    uint64_t type_idx_leb = 0;
    if (!read_leb((uint8_t **)&p, end, 32, false, &type_idx_leb, error_buf, error_buf_size)) {
...
static bool parse_borrow_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    (*out)->def_val.borrow = wasm_runtime_malloc(sizeof(WASMComponentBorrowType));
    if (!(*out)->def_val.borrow) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for borrow type");
        return false;
    }
    ...
    if (!read_leb((uint8_t **)&p, end, 32, false, &type_idx_leb, error_buf, error_buf_size)) {
        ...
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse borrow type");
        return false;
    }
...
static bool parse_stream_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if (!(*out)->def_val.stream) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for stream type");
        return false;
    }
    ...
    if (has_element_type == WASM_COMP_OPTIONAL_TRUE) {
        // Allocate the element type
        (*out)->def_val.stream->element_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
        if (!(*out)->def_val.stream->element_type) {
            wasm_runtime_free((*out)->def_val.stream);
            (*out)->def_val.stream = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for stream element type");
            return false;
        }
        
        // Initialize element type to zero
        memset((*out)->def_val.stream->element_type, 0, sizeof(WASMComponentValueType));
        
        if (!parse_valtype(&p, end, (*out)->def_val.stream->element_type, error_buf, error_buf_size)) {
            wasm_runtime_free((*out)->def_val.stream->element_type);
            wasm_runtime_free((*out)->def_val.stream);
            (*out)->def_val.stream = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to parse stream element type");
            return false;
        }
    } else if (has_element_type != WASM_COMP_OPTIONAL_FALSE) {
        set_error_buf_ex(error_buf, error_buf_size, "Malformed binary: invalid optional tag 0x%02x", has_element_type);
        return false;
    }
...
static bool parse_future_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    (*out)->def_val.future = wasm_runtime_malloc(sizeof(WASMComponentFutureType));
    if (!(*out)->def_val.future) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for future type");
        return false;
    }
    ...
    if (has_element_type == WASM_COMP_OPTIONAL_TRUE) {
        // Allocate the element type
        (*out)->def_val.future->element_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
        if (!(*out)->def_val.future->element_type) {
            wasm_runtime_free((*out)->def_val.future);
            (*out)->def_val.future = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for future element type");
            return false;
        }
        
        // Initialize element type to zero
        memset((*out)->def_val.future->element_type, 0, sizeof(WASMComponentValueType));
        
        if (!parse_valtype(&p, end, (*out)->def_val.future->element_type, error_buf, error_buf_size)) {
            wasm_runtime_free((*out)->def_val.future->element_type);
            wasm_runtime_free((*out)->def_val.future);
            (*out)->def_val.future = NULL;
            set_error_buf_ex(error_buf, error_buf_size, "Failed to parse future element type");
            return false;
        }
    } else if (has_element_type != WASM_COMP_OPTIONAL_FALSE) {
        set_error_buf_ex(error_buf, error_buf_size, "Malformed binary: invalid optional tag 0x%02x", has_element_type);
...
static bool parse_list_type_with_len(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if (!(*out)->def_val.list_len) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for list_len type");
        return false;
    }
    
    // Allocate the element_type
    (*out)->def_val.list_len->element_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
    if (!(*out)->def_val.list_len->element_type) {
        wasm_runtime_free((*out)->def_val.list_len);
        (*out)->def_val.list_len = NULL;
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for list_len element type");
        return false;
    }
    
    // Parse the length
    uint64_t len_leb = 0;
    if (!read_leb((uint8_t **)&p, end, 32, false, &len_leb, error_buf, error_buf_size)) {
    ...
    if (len_leb == 0) {
        set_error_buf_ex(error_buf, error_buf_size, "Fixed-size list length must be greater than 0");
        wasm_runtime_free((*out)->def_val.list_len->element_type);
        wasm_runtime_free((*out)->def_val.list_len);
        (*out)->def_val.list_len = NULL;
        return false;
    ...
    if (!parse_valtype(&p, end, (*out)->def_val.list_len->element_type, error_buf, error_buf_size)) {
        wasm_runtime_free((*out)->def_val.list_len->element_type);
        wasm_runtime_free((*out)->def_val.list_len);
        (*out)->def_val.list_len = NULL;
        return false;
...
static bool parse_list_type(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out || !end) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid payload or output pointer");
        return false;
    }
    ...
    if (!(*out)->def_val.list) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for list type");
        return false;
    }
    
    // Allocate the element_type
    (*out)->def_val.list->element_type = wasm_runtime_malloc(sizeof(WASMComponentValueType));
    if (!(*out)->def_val.list->element_type) {
        wasm_runtime_free((*out)->def_val.list);
        (*out)->def_val.list = NULL;
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for list element type");
        return false;
    }
    
    if (!parse_valtype(&p, end, (*out)->def_val.list->element_type, error_buf, error_buf_size)) {
...
static void free_defvaltype(WASMComponentDefValType *def_val_type) {
    if (!def_val_type) {
        return;
    }
        ...
        case WASM_COMP_DEF_VAL_RECORD: {
            if (def_val_type->def_val.record) {
                if (def_val_type->def_val.record->fields) {
                    for (uint32_t i = 0; i < def_val_type->def_val.record->count; i++) {
                        free_labelvaltype(&def_val_type->def_val.record->fields[i]);
                    }
        ...
        case WASM_COMP_DEF_VAL_VARIANT: {
            if (def_val_type->def_val.variant) {
                if (def_val_type->def_val.variant->cases) {
                    for (uint32_t i = 0; i < def_val_type->def_val.variant->count; i++) {
        ...
        case WASM_COMP_DEF_VAL_LIST_LEN: {
            if (def_val_type->def_val.list_len) {
                if (def_val_type->def_val.list_len->element_type) {
                    wasm_runtime_free(def_val_type->def_val.list_len->element_type);
        ...
        case WASM_COMP_DEF_VAL_TUPLE: {
            if (def_val_type->def_val.tuple) {
                if (def_val_type->def_val.tuple->element_types) {
                    wasm_runtime_free(def_val_type->def_val.tuple->element_types);
        ...
        case WASM_COMP_DEF_VAL_FLAGS: {
            if (def_val_type->def_val.flag) {
                if (def_val_type->def_val.flag->flags) {
                    for (uint32_t i = 0; i < def_val_type->def_val.flag->count; i++) {
        ...
        case WASM_COMP_DEF_VAL_ENUM: {
            if (def_val_type->def_val.enum_type) {
                if (def_val_type->def_val.enum_type->labels) {
                    for (uint32_t i = 0; i < def_val_type->def_val.enum_type->count; i++) {
                        free_label_prime(&def_val_type->def_val.enum_type->labels[i]);
                    }
        ...
        case WASM_COMP_DEF_VAL_OPTION: {
            if (def_val_type->def_val.option) {
                if (def_val_type->def_val.option->element_type) {
                    wasm_runtime_free(def_val_type->def_val.option->element_type);
        ...
        case WASM_COMP_DEF_VAL_RESULT: {
            if (def_val_type->def_val.result) {
                if (def_val_type->def_val.result->result_type) {
                    wasm_runtime_free(def_val_type->def_val.result->result_type);
        ...
        case WASM_COMP_DEF_VAL_STREAM: {
            if (def_val_type->def_val.stream) {
                if (def_val_type->def_val.stream->element_type) {
                    wasm_runtime_free(def_val_type->def_val.stream->element_type);
        ...
        case WASM_COMP_DEF_VAL_FUTURE: {
            if (def_val_type->def_val.future) {
                if (def_val_type->def_val.future->element_type) {
                    wasm_runtime_free(def_val_type->def_val.future->element_type);
...
static bool parse_defvaltype(const uint8_t **payload, const uint8_t *end, WASMComponentDefValType **out, char *error_buf, uint32_t error_buf_size) {
    ...
    *out = wasm_runtime_malloc(sizeof(WASMComponentDefValType));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for def_val_type");
        return false;
    }
    ...
    switch (tag) {
        ...
        case WASM_COMP_DEF_VAL_LIST_LEN:
            if (!parse_list_type_with_len(&p, end, out, error_buf, error_buf_size)) {
                wasm_runtime_free(*out);
                *out = NULL;
                return false;
            }
        ...
        default: {
            set_error_buf_ex(error_buf, error_buf_size, "Invalid defvaltype tag: 0x%02x", tag);
            wasm_runtime_free(*out);
            *out = NULL;
            return false;
        }
...
static bool parse_param_list(const uint8_t **payload, const uint8_t *end, WASMComponentParamList **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;

    // Allocate memory for the param list structure
    *out = wasm_runtime_malloc(sizeof(WASMComponentParamList));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for param list");
        return false;
    }
    ...
    if (!read_leb((uint8_t **)&p, end, 32, false, &param_count_leb, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse param count");
        return false;
    }
    ...
    if (param_count > 0) {
        (*out)->params = wasm_runtime_malloc(sizeof(WASMComponentLabelValType) * param_count);
        if (!(*out)->params) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for param list");
            return false;
        }
        ...
        for (uint32_t i = 0; i < param_count; i++) {
            if (!parse_labelvaltype(&p, end, &(*out)->params[i], error_buf, error_buf_size)) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to parse param %d", i);
                return false;
            }
        }
    } else {
...
bool parse_func_type(const uint8_t **payload, const uint8_t *end, WASMComponentFuncType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out) {
        return false;
    }
    ...
    *out = wasm_runtime_malloc(sizeof(WASMComponentFuncType));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for func_type");
        return false;
    }
    memset(*out, 0, sizeof(WASMComponentFuncType));
    
    // Parse the param list
    if (!parse_param_list(&p, end, &(*out)->params, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse param list");
        return false;
    }
    
    // Parse the result list
    if (!parse_result_list(&p, end, &(*out)->results, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse result list");
        return false;
    }
...
bool parse_component_decl_import_decl(const uint8_t **payload, const uint8_t *end, WASMComponentImportDecl **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;

    // Allocate memory for the import decl
    *out = wasm_runtime_malloc(sizeof(WASMComponentImportDecl));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for import decl");
        return false;
    }
    ...
    if (!status) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse component name for import");
    ...
    if (!extern_desc) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for extern desc");
        wasm_runtime_free(import_name);
        return false;
    }
    ...
    if (!parse_extern_desc(&p, end, extern_desc, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse extern desc");
...
bool parse_component_decl_core_type(const uint8_t **payload, const uint8_t *end, WASMComponentCoreType **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;

    // Allocate memory for the core type
    *out = wasm_runtime_malloc(sizeof(WASMComponentCoreType));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for core type");
        return false;
    }
    memset(*out, 0, sizeof(WASMComponentCoreType));

    // Allocate memory fore the core def type
    WASMComponentCoreDefType *deftype = wasm_runtime_malloc(sizeof(WASMComponentCoreDefType));
    if (!deftype) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for core def type");
        return false;
    }
    ...
    if (p < end) {
        uint8_t b0 = *p;
        bool looks_like_inline = (b0 == 0x50) /* moduletype */
                              || (b0 == 0x4E) /* rec group (GC, unsupported here) */
                              || (b0 == 0x4F) /* final subtype (GC, unsupported here) */
                              || (b0 == 0x5E) || (b0 == 0x5F) || (b0 == 0x60) /* comptype prefixes (GC) */
                              || (b0 == 0x00 && (p + 1) < end &&
                                  (*(p + 1) == 0x50 || *(p + 1) == 0x4E || *(p + 1) == 0x4F ||
                                   *(p + 1) == 0x5E || *(p + 1) == 0x5F || *(p + 1) == 0x60));
        ...
        if (!looks_like_inline) {
            ...
            // The index %llu will need to be resolved later against the core type section
            (*out)->deftype = NULL;
            
            // IMPORTANT: After parsing a core type index, we need to continue parsing
            // the rest of the instancedecl. The core type index is just the first part.
            // We'll let the caller continue parsing from this point.
            *payload = p;
            return true;
        }
    }

    // Parse the core type inline (Binary.md: instancedecl 0x00 t:<core:type> where core:type ::= dt:<core:deftype>)
...
bool parse_component_decl_type(const uint8_t **payload, const uint8_t *end, WASMComponentTypes **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;
...
bool parse_component_decl_alias(const uint8_t **payload, const uint8_t *end, WASMComponentAliasDefinition **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;

    // Allocate memory for the alias definition
    *out = wasm_runtime_malloc(sizeof(WASMComponentAliasDefinition));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for alias definition");
        return false;
    }
...
bool parse_component_decl_export(const uint8_t **payload, const uint8_t *end, WASMComponentComponentDeclExport **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;
    ...
    *out = wasm_runtime_malloc(sizeof(WASMComponentComponentDeclExport));
    ...
    if (!status) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse component name for export");
...
bool parse_component_instance_decl(const uint8_t **payload, const uint8_t *end, WASMComponentInstDecl **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;

    // Allocate memory for the instance decl
    *out = wasm_runtime_malloc(sizeof(WASMComponentInstDecl));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for instance decl");
        return false;
    }
    ...
    switch (tag) {
        case WASM_COMP_COMPONENT_DECL_CORE_TYPE: {
            if (!parse_component_decl_core_type(&p, end, &(*out)->decl.core_type, error_buf, error_buf_size)) {
                return false;
            }
        ...
        case WASM_COMP_COMPONENT_DECL_TYPE: {
            if (!parse_component_decl_type(&p, end, &(*out)->decl.type, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        case WASM_COMP_COMPONENT_DECL_ALIAS: {
            if (!parse_component_decl_alias(&p, end, &(*out)->decl.alias, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        case WASM_COMP_COMPONENT_DECL_EXPORT: {
            if (!parse_component_decl_export(&p, end, &(*out)->decl.export_decl, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        default: {
            set_error_buf_ex(error_buf, error_buf_size, "Invalid instance decl tag: 0x%02x", tag);
            return false;
        }
    }
...
bool parse_component_decl(const uint8_t **payload, const uint8_t *end, WASMComponentComponentDecl **out, char *error_buf, uint32_t error_buf_size) {
    ...
    if (tag == WASM_COMP_COMPONENT_DECL_IMPORT) {
        p++;
        if (!parse_component_decl_import_decl(&p, end, &(*out)->decl.import_decl, error_buf, error_buf_size)) {
            return false;
        }
    } else {
        if (!parse_component_instance_decl(&p, end, &(*out)->decl.instance_decl, error_buf, error_buf_size)) {
            return false;
        }
    }
...
bool parse_component_type(const uint8_t **payload, const uint8_t *end, WASMComponentComponentType **out, char *error_buf, uint32_t error_buf_size) {
    ...
    *out = wasm_runtime_malloc(sizeof(WASMComponentComponentType));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for component type");
        return false;
    }
    ...
    if (!read_leb((uint8_t **)&p, end, 32, false, &component_count_leb, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse component count");
        return false;
    }
    ...
    if (component_count > 0) {
        (*out)->component_decls = wasm_runtime_malloc(sizeof(WASMComponentComponentDecl) * component_count);
        if (!(*out)->component_decls) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for component list");
            return false;
        }

        // Parse the component list
        for (uint32_t i = 0; i < component_count; i++) {
            WASMComponentComponentDecl *component_decl = wasm_runtime_malloc(sizeof(WASMComponentComponentDecl));
            if (!component_decl) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for component decl");
                return false;
            }
            memset(component_decl, 0, sizeof(WASMComponentComponentDecl));

            if (!parse_component_decl(&p, end, &component_decl, error_buf, error_buf_size)) {
                set_error_buf_ex(error_buf, error_buf_size, "Failed to parse component %d", i);
...
bool parse_component_instance_type(const uint8_t **payload, const uint8_t *end, WASMComponentInstType **out, char *error_buf, uint32_t error_buf_size) {
    if (!payload || !*payload || !out) {
        return false;
    }
    ...
    *out = wasm_runtime_malloc(sizeof(WASMComponentInstType));
    ...
    if (!read_leb((uint8_t **)&p, end, 32, false, &instance_count_leb, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse instance count");
        goto fail;
    }
    ...
    if (instance_count > 0) {
        instance_decls = wasm_runtime_malloc(sizeof(WASMComponentInstDecl) * instance_count);
        if (!instance_decls) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for instance list");
            goto fail;
        }
        ...
        for (uint32_t i = 0; i < instance_count; i++) {
            if (p >= end) {
                set_error_buf_ex(error_buf, error_buf_size, "Unexpected end of buffer at instancedecl %d", i);
                goto fail;
            }

            instance_decl = NULL;
            if (!parse_component_instance_decl(&p, end, &instance_decl, error_buf, error_buf_size)) {
                goto fail;
            }
...
bool parse_resource_type_sync(const uint8_t **payload, const uint8_t *end, WASMComponentResourceTypeSync **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;

    // Allocate memory for the resource type sync
    *out = wasm_runtime_malloc(sizeof(WASMComponentResourceTypeSync));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for resource type sync");
        return false;
    }
    ...
    if (has_result_type == WASM_COMP_OPTIONAL_TRUE) {
        ...
        if (!read_leb((uint8_t **)&p, end, 32, false, &dtor_func_idx_leb, error_buf, error_buf_size)) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to parse resource type sync");
            return false;
        }
        (*out)->has_dtor = true;
...
bool parse_resource_type_async(const uint8_t **payload, const uint8_t *end, WASMComponentResourceTypeAsync **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;
    ...
    *out = wasm_runtime_malloc(sizeof(WASMComponentResourceTypeAsync));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for resource type async");
        return false;
    }
    ...
    uint64_t dtor_func_idx_leb = 0;
    if (!read_leb((uint8_t **)&p, end, 32, false, &dtor_func_idx_leb, error_buf, error_buf_size)) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to parse resource type async");
        return false;
    }
    (*out)->dtor_func_idx = (uint32_t)dtor_func_idx_leb;
    ...
    if (has_result_type == WASM_COMP_OPTIONAL_TRUE) {
        ...
        if (!read_leb((uint8_t **)&p, end, 32, false, &callback_func_idx_leb, error_buf, error_buf_size)) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to parse resource type async");
            return false;
        }
...
bool parse_resource_type(const uint8_t **payload, const uint8_t *end, WASMComponentResourceType **out, char *error_buf, uint32_t error_buf_size) {
    const uint8_t *p = *payload;

    // Allocate memory for the resource type
    *out = wasm_runtime_malloc(sizeof(WASMComponentResourceType));
    if (!*out) {
        set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for resource type");
        return false;
    }
    ...
    if (*p++ != WASM_COMP_RESOURCE_REP_I32) {
        set_error_buf_ex(error_buf, error_buf_size, "Invalid resource type rep: 0x%02x", *p);
        return false;
    }

    switch (tag) {
        case WASM_COMP_RESOURCE_TYPE_SYNC: {
            if (!parse_resource_type_sync(&p, end, &(*out)->resource.sync, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        ...
        case WASM_COMP_RESOURCE_TYPE_ASYNC: {
            if (!parse_resource_type_async(&p, end, &(*out)->resource.async, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        ...
        default: {
            set_error_buf_ex(error_buf, error_buf_size, "Invalid resource type tag: 0x%02x", tag);
            return false;
        }
    }
...
bool parse_single_type(const uint8_t **payload, const uint8_t *end, WASMComponentTypes *out, char *error_buf, uint32_t error_buf_size) {
    ...
    switch (type_tag) {
        case WASM_COMP_DEF_TYPE: {
            if (!parse_defvaltype(&p, end, &out->type.def_val_type, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        ...
        case WASM_COMP_FUNC_TYPE: {
            if (!parse_func_type(&p, end, &out->type.func_type, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        ...
        case WASM_COMP_COMPONENT_TYPE: {
            ...
            if (!parse_component_type(&p, end, &out->type.component_type, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        ...
        case WASM_COMP_INSTANCE_TYPE: {
            ...
            if (!parse_component_instance_type(&p, end, &out->type.instance_type, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        ...
        case WASM_COMP_RESOURCE_TYPE_ASYNC: {
            ...
            // Note: We already consumed the leading tag byte above. Rewind so the
            // resource parser can read the tag and rep as specified by Binary.md.
            p--;
            if (!parse_resource_type(&p, end, &out->type.resource_type, error_buf, error_buf_size)) {
                return false;
            }
            break;
        }
        ...
        default: {
            set_error_buf_ex(error_buf, error_buf_size, "Invalid type tag: 0x%02x", tag);
            return false;
        }
    }
...
bool wasm_component_parse_types_section(const uint8_t **payload, uint32_t payload_len, WASMComponentTypeSection *out, char *error_buf, uint32_t error_buf_size, uint32_t *consumed_len) {
    ...
    if (!read_leb((uint8_t **)&p, end, 32, false, &type_count_leb, error_buf, error_buf_size)) {
    ...
    if (type_count > 0) {
        ...
        out->types = wasm_runtime_malloc(sizeof(WASMComponentTypes) * type_count);
        if (!out->types) {
            set_error_buf_ex(error_buf, error_buf_size, "Failed to allocate memory for types array");
            if (consumed_len) *consumed_len = (uint32_t)(p - *payload);
            return false;
        }
        ...
        for (uint32_t i = 0; i < type_count; ++i) {
            if (!parse_single_type(&p, end, &out->types[i], error_buf, error_buf_size)) {
                ...
                switch (decl->decl.alias->alias_target_type) {
                    case WASM_COMP_ALIAS_TARGET_EXPORT:
                        if (decl->decl.alias->target.exported.name) {
                            free_core_name(decl->decl.alias->target.exported.name);
                            wasm_runtime_free(decl->decl.alias->target.exported.name);
                    ...
                    case WASM_COMP_ALIAS_TARGET_CORE_EXPORT:
                        if (decl->decl.alias->target.core_exported.name) {
                            free_core_name(decl->decl.alias->target.core_exported.name);
                            wasm_runtime_free(decl->decl.alias->target.core_exported.name);
        ...
        case WASM_COMP_COMPONENT_DECL_INSTANCE_EXPORTDECL:
            if (decl->decl.export_decl) {
                if (decl->decl.export_decl->export_name) {
                    free_component_export_name(decl->decl.export_decl->export_name);
...
static void free_component_decl(WASMComponentComponentDecl *decl) {
    if (!decl) return;
    switch (decl->tag) {
        case WASM_COMP_COMPONENT_DECL_IMPORT:
            if (decl->decl.import_decl) {
                if (decl->decl.import_decl->import_name) {
                    free_component_import_name(decl->decl.import_decl->import_name);
            ...
            break;
        default:
            // Instance decl branch handles tags 0x00/0x01/0x02/0x04 inside nested decl
...
static void free_resource_type(WASMComponentResourceType *rt) {
    if (!rt) return;
    switch (rt->tag) {
        case WASM_COMP_RESOURCE_TYPE_SYNC:
            if (rt->resource.sync) {
            ...
            if (type->type.func_type) {
                if (type->type.func_type->params) {
                    if (type->type.func_type->params->params) {
                        for (uint32_t j = 0; j < type->type.func_type->params->count; j++) {
                            if (type->type.func_type->params->params[j].label) {
                                free_core_name(type->type.func_type->params->params[j].label);
                                wasm_runtime_free(type->type.func_type->params->params[j].label);
                            }
                            if (type->type.func_type->params->params[j].value_type) {
                                wasm_runtime_free(type->type.func_type->params->params[j].value_type);
                            }
                        }
                ...
                if (type->type.func_type->results) {
                    if (type->type.func_type->results->results) {
                        wasm_runtime_free(type->type.func_type->results->results);
                    }
        ...
        case WASM_COMP_COMPONENT_TYPE:
            if (type->type.component_type) {
                if (type->type.component_type->component_decls) {
                    for (uint32_t k = 0; k < type->type.component_type->count; k++) {
                        free_component_decl(&type->type.component_type->component_decls[k]);
                    }
                    wasm_runtime_free(type->type.component_type->component_decls);
        ...
        case WASM_COMP_INSTANCE_TYPE:
            if (type->type.instance_type) {
                if (type->type.instance_type->instance_decls) {
                    for (uint32_t k = 0; k < type->type.instance_type->count; k++) {
                        free_component_instance_decl(&type->type.instance_type->instance_decls[k]);
                    }
