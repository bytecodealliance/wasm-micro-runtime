...
#include "../common/wasm_multimodules_program.h"
...
typedef struct LoadingModuleElem {
    bh_list_link l;
    WASMModule * module;
} LoadingModuleElem;
...
inline static WASMModuleInstance *
        ...
        for (i = 0; i < count; i++) {
            if (memories[i]) {
                ...
                if (module_inst->program &&
                    module_inst != (WASMModuleInstance*)module_inst->program->root_module_inst)
                    continue;
#endif
#if WASM_ENABLE_MULTI_MODULE != 0
                   ...
                   uint32 error_buf_size)
{
    ...
    if (init_page_count == max_page_count && init_page_count == 1) {
        ...
        if (module->aux_heap_base_global_index != (uint32)-1
            ...
            global_addr = module_inst->globals[global_idx].data;
...
#if WASM_ENABLE_DYNAMIC_LINKING != 0
static WASMMemoryInstance **
dependency_memories_instantiate(const WASMModule *module,
                     WASMModuleInstance *root_module_inst)
{
    uint32 memory_count =
    ...
    memories[0] = root_module_inst->memories[0];
...
dependency_tables_instantiate(const WASMModule *module,
                    ...
                    WASMModuleInstance *root_module_inst)
{
    ...
    uint32 table_index = 0, cur_size = 0, max_size = 0;
    uint64 total_data_size = 0;
    WASMTableInstance **tables, *table = NULL;
    WASMProgramInstance * program = module_inst->program;

    // have to establish a table to keep the export function pointer
    if (!table_count)
        table_count = 1;

    bh_assert(table_count == 1);

    total_data_size = sizeof(WASMTableInstance*) * (uint64)table_count;

    if (!(tables = runtime_malloc(total_data_size,
                                  program->error_buf, program->error_buf_size))) {
        return NULL;
    }
    ...
    for (uint32 i = 0; i < module->import_table_count; i++, import++) {
        /* it is a built-in table, every module has its own */
        max_size = import->u.table.possible_grow?
                import->u.table.max_size:import->u.table.init_size;
        if (program->config.use_tbl_as_cache) {
            max_size += module_inst->export_func_count;
            cur_size = import->u.table.init_size + module_inst->export_func_count;
        }

        if (max_size > TABLE_SPACE_SLOT_SIZE) {
            return NULL;
        }

        total_data_size = offsetof(WASMTableInstance, base_addr);
        total_data_size += sizeof(uint32) * (uint64)max_size;

        if (!(table = tables[table_index++] = runtime_malloc
                    (total_data_size , program->error_buf, program->error_buf_size))) {
            tables_deinstantiate(tables, table_count);
            return NULL;
        }

        /* Set all elements to -1 to mark them as uninitialized elements */
        memset(table, -1, (uint32)total_data_size);
    ...
    if (!table_index && program->config.use_tbl_as_cache) {
        max_size = module_inst->export_func_count;
        cur_size = module_inst->export_func_count;
        ...
        table->elem_type = VALUE_TYPE_FUNCREF;
        table->cur_size = cur_size;
        table->max_size = max_size;
    }
                   ...
                   char *error_buf, uint32 error_buf_size)
{
    ...
    for (i = 0; i < module->import_table_count; i++, import++) {
...
#endif
        {
            ...
            max_size = max_size_fixed;
            cur_size = import->u.table.init_size;
            ...
            if (runtime->config.need_load_dependencies) {
                ...
                if (module->dylink_section) {
                    if (wasm_program_is_root_module((WASMModuleInstanceCommon*)module_inst)) {
                        max_size ++;
                        cur_size ++;
                    }
                }
...
#endif
            total_size += sizeof(uint32) * max_size;
        }
                      ...
                      char *error_buf, uint32 error_buf_size)
{
    ...
    for (i = 0; i < module->import_function_count; i++, import++) {
        function->module_inst = module_inst;
            ...
            if (function->import_module_inst) {
                function->import_func_inst =
                  wasm_lookup_function(function->import_module_inst,
                                       import->u.function.field_name->str, NULL);
            }
        }
#endif /* WASM_ENABLE_MULTI_MODULE */
        function->u.func_import = &import->u.function;
        function->func_type = import->u.function.func_type;
        function->local_cell_num = 0;
        function->local_count = 0;
        function->local_types = NULL;

#if WASM_ENABLE_DYNAMIC_LINKING != 0
        // reset malloc/free/realloc func pointer because it's likely that they are not correct.
        // will be re-linked in resolving stage later
        if (program && program->root_module_inst &&
            (WASMModuleInstance*)program->root_module_inst != module_inst &&
            program->config.import_memop_mode == FROM_ROOT &&
            wasm_runtime_is_memop_symbol(runtime, import->u.function.field_name) &&
            import->u.function.module_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_env)) {
            import->u.function.func_ptr_linked = NULL;
        }
#endif
    ...
    for (i = 0; i < module->function_count; i++) {
        ...
        function->func_type = module->functions[i]->func_type;
    ...
    for (i = 0; i < module->import_global_count; i++, import++) {
        ...
        if (global_import->import_module) {
            ...
            if (!(global->import_global_inst = wasm_lookup_global(
                    global->import_module_inst, global_import->field_name->str))) {
                set_error_buf(error_buf, error_buf_size, "unknown global");
                return NULL;
            }
        ...
        {
#if WASM_ENABLE_DYNAMIC_LINKING != 0
            if (global_import->module_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_env)) {
                if (global_import->field_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_var_stack_pointer)) {
                    bh_assert(global->is_mutable == true);
                    global->initial_value.u32 = module_inst->init_globals.stack_pointer;
                    global_import->is_linked = true;
                    //global->data = (uint8*)&module_inst->init_globals.stack_pointer;
                } else if (global_import->field_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_var_memory_base)) {
                    bh_assert(global->is_mutable == false);
                    global->initial_value.u32 = module_inst->init_globals.memory_base;
                    global_import->is_linked = true;
                } else if (global_import->field_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_var_table_base)) {
                    bh_assert(global->is_mutable == false);
                    global->initial_value.u32 = module_inst->init_globals.table_base;
                    global_import->is_linked = true;
                } else
#endif
                {
                    // TODO: link other env global variables if exists
                ...
                // TODO: link other non-env global variables if exists
                // lazy link
                if (global_import->is_linked)
                    bh_memcpy_s(&(global->initial_value), sizeof(WASMValue),
                                    &(global_import->global_data_linked),
                                    sizeof(WASMValue));
            }
 ...
 */
static void
get_export_count(const WASMModule *module, WASMModuleInstance * module_inst)
{
    WASMExport *export = module->exports;

    for (uint32 i = 0; i < module->export_count; i++, export++) {
        if (export->kind == EXPORT_KIND_FUNC)
            module_inst->export_func_count ++;
#if WASM_ENABLE_MULTI_MODULE != 0 || WASM_ENABLE_DYNAMIC_LINKING != 0
        else if (export->kind == EXPORT_KIND_TABLE)
            module_inst->export_tab_count ++;
        else if (export->kind == EXPORT_KIND_MEMORY)
...
execute_post_inst_function(WASMModuleInstance *module_inst)
{
    ...
    // https://github.com/emscripten-core/emscripten/issues/13893
    for (i = 0; i < module_inst->export_func_count; i++)
        if (!strcmp(module_inst->export_functions[i].name, "__post_instantiate") ||
            !strcmp(module_inst->export_functions[i].name, "__wasm_call_ctors")) {
...
execute_start_function(WASMModuleInstance *module_inst)
{
    ...
    bh_assert(!func->is_import_func && func->func_type->param_cell_num == 0
              && func->func_type->ret_cell_num == 0);
                       ...
                       uint32 error_buf_size)
{
    ...
    while (sub_module_list_node) {
        ...
        WASMModuleInstance *sub_module_inst =
                wasm_instantiate(NULL, sub_module, false, stack_size, heap_size,
                                 error_buf, error_buf_size);
        if (!sub_module_inst) {
    ...
    for (i = 0; i < module->import_function_count; i++) {
        ...
        if (!func->func_ptr_linked
            ...
            LOG_WARNING("warning: failed to link import function (%s, %s)",
                        func->module_name->str, func->field_name->str);
#else
    ...
    for (i = 0; i < module->import_global_count; i++) {
        ...
        if (!global->is_linked) {
            ...
            LOG_DEBUG("warning: failed to link import global (%s, %s)",
                      global->module_name->str, global->field_name->str);
#else
            /* do nothing to avoid confused message */
#endif /* WASM_ENABLE_WAMR_COMPILER == 0 */
#endif /* WASM_ENABLE_SPEC_TEST != 0 */
...
#if WASM_ENABLE_DYNAMIC_LINKING != 0
static WASMGlobalInstance *
dependency_globals_instantiate(const WASMModule *module,
                    WASMModuleInstance *module_inst,
                    uint32 *global_data_size_ptr)
{
    WASMImport *import;
    uint32 global_data_offset = 0;
    uint32 i, global_count =
    ...
    for (i = 0; i < module->import_global_count; i++, import++) {
        ...
        if (global_import->module_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_env)) {
            if (global_import->field_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_var_stack_pointer)) {
                if (!wasm_program_link_sp_wasm_globals(program, global, global_import->field_name)) {
                ...
                //global_import->is_linked = true;
                //global_import->global_data_linked.u32 = module_inst->init_globals.memory_base;
                bh_assert(global->is_mutable == false);
                global->initial_value.u32 = module_inst->init_globals.memory_base;
            } else if (global_import->field_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_var_table_base)) {
                bh_assert(global->is_mutable == false);
        ...
        } else if (global_import->module_name == CONST_STR_POOL_DESC(runtime, WAMR_CSP_GOT_func)) {
                if (!wasm_program_link_got_globals(program,
                        module_inst, global, global_import->field_name)) {
                    set_error_buf(program->error_buf, program->error_buf_size,
    ...
    bh_assert((uint32)(global - globals) == global_count);
    *global_data_size_ptr = global_data_offset;
    (void)module_inst;
    return globals;
}
                 ...
                 uint32 stack_size, DependencyModuleInitGlobals * init_globals)
{
    ...
    uint32 global_count, global_data_size = 0, i = 0, table_space_start = 0;
    uint32 base_offset, length;
    uint8 *global_data, *global_data_end;

    /* Allocate the memory */
    if (!(module_inst = wasm_runtime_malloc(sizeof(WASMModuleInstance)))) {
        return NULL;
    }
    ...
    bh_assert(module->memory_count == 0 && module->import_memory_count == 1);
    ...
    /* export */
    get_export_count(module, module_inst);

    // use root module memory to instantiate
    module_inst->memories = dependency_memories_instantiate(module, (WASMModuleInstance*)program->root_module_inst);
    ...
    module_inst->tables = dependency_tables_instantiate(module, module_inst, (WASMModuleInstance*)program->root_module_inst);


    module_inst->default_table =
      module_inst->table_count ? module_inst->tables[0] : NULL;

    table_space_start = wasm_program_alloc_table_space_by_size(module_inst->inst_id, module_inst->default_table->max_size);
    ...
    if (module_inst->function_count > 0) {
        module_inst->functions = functions_instantiate(module, module_inst,
                                                    program->error_buf, program->error_buf_size);
    }

    if (module_inst->export_func_count > 0) {
        module_inst->export_functions = export_functions_instantiate(module, module_inst,
                                                    module_inst->export_func_count,
                                                    program->error_buf, program->error_buf_size);
    }
    ...
    if (global_count
        && !(globals = dependency_globals_instantiate(module, module_inst,
                                           &global_data_size))) {
        goto fail;
    }
    ...
    if (global_count > 0) {
        if (!(module_inst->global_data = runtime_malloc
                    (global_data_size, program->error_buf,
    ...
    for (i = 0; module_inst->table_count > 0 && i < module->table_seg_count;
        ...
        if (table_seg->base_offset.init_expr_type
            ...
            table_base = globals[table_seg->base_offset.u.global_index].initial_value.i32;
        }

        /* check offset since length might negative */
        if (table_base > (module_inst->init_globals.table_base + table->cur_size)) {
            LOG_DEBUG("base_offset(%d) > table->cur_size(%d)",
                      table_base, module_inst->init_globals.table_base + table->cur_size);
#if WASM_ENABLE_REF_TYPES != 0
            set_error_buf(program->error_buf, program->error_buf_size,
        ...
        if (table_base + length > (module_inst->init_globals.table_base + table->cur_size)) {
            LOG_DEBUG("base_offset(%d) + length(%d)> table->cur_size(%d)",
                      table_base, length, module_inst->init_globals.table_base + table->cur_size);
#if WASM_ENABLE_REF_TYPES != 0
            set_error_buf(program->error_buf, program->error_buf_size,
                          "out of bounds table access");
#else
         ...
         */
        bh_memcpy_s(
          table_data + table_base - module_inst->init_globals.table_base,
          (uint32)((module_inst->init_globals.table_base + table->cur_size - table_base)
                   * sizeof(uint32)),
    ...
    /* Execute __wasm_call_ctors function */
    // Note: global relocs has to be run by each thread.
...
fail:
    wasm_deinstantiate(module_inst, true);
    return NULL;
}
#endif

bool
init_globals(WASMModule * module, WASMModuleInstance * module_inst,
            char *error_buf, uint32 error_buf_size)
{
    uint32 global_count = 0, global_data_size = 0, i;
    WASMGlobalInstance *globals = NULL, *global;
    uint8 *global_data, *global_data_end;
                ...
                char *error_buf, uint32 error_buf_size)
{
    ...
    module_inst->init_globals.table_base = 1; // 0 is reserved for NULL
...
#if WASM_ENABLE_DYNAMIC_LINKING != 0
    if (module->dylink_section) {
        module_inst->init_globals.table_size = module->dylink_section->table_size +
            module_inst->export_func_count;
    }
#endif

    if (!module->dylink_section) {
        if (!init_globals(module, module_inst, error_buf, error_buf_size))
            goto fail;
    }
    ...
    if (module_inst->memory_count)
        module_inst->init_globals.stack_pointer = module_inst->memories[0]->heap_data -
    ...
    for (i = 0; i < module->data_seg_count; i++) {
        ...
        if (data_seg->base_offset.init_expr_type == INIT_EXPR_TYPE_GET_GLOBAL) {
            ...
            if (!module_inst->globals
                || module_inst->globals[data_seg->base_offset.u.global_index].type
                     != VALUE_TYPE_I32) {
                set_error_buf(error_buf, error_buf_size,
                              "data segment does not fit");
                goto fail;
            ...
            base_offset = module_inst->globals[data_seg->base_offset.u.global_index]
                .initial_value.i32;
            //data_seg->base_offset.u.i32 =
            //    module_inst->globals[data_seg->base_offset.u.global_index]
            //    .initial_value.i32;
        } else {
            base_offset = (uint32)data_seg->base_offset.u.i32;
        }
    ...
    module_inst->default_table =
      module_inst->table_count ? module_inst->tables[0] : NULL;
#if WASM_ENABLE_DYNAMIC_LINKING != 0
    bh_assert(!module_inst->program ||
        (module_inst->program && module_inst->inst_id == 1));

    //wasm_program_alloc_table_space_by_table(module_inst, module_inst->default_table);
#endif
    /* in case there is no table */
    for (i = 0; module_inst->table_count > 0 && i < module->table_seg_count;
         i++) {
        ...
        if (table_seg->base_offset.init_expr_type
            ...
            if (!module_inst->globals
                || module_inst->globals[table_seg->base_offset.u.global_index].type
                   != VALUE_TYPE_I32) {
                set_error_buf(error_buf, error_buf_size,
                              "elements segment does not fit");
                goto fail;
            ...
            table_seg->base_offset.u.i32 =
              module_inst->globals[table_seg->base_offset.u.global_index].initial_value.i32;
        }
    ...
    if (program) {
        if (!wasm_program_insert_module_inst_by_name(program,
                                                (WASMModuleInstanceCommon*)module_inst,
                                                module_inst->module->module_name))
            goto fail;
    }
#endif
...
wasm_dump_perf_profiling(const WASMModuleInstance *module_inst)
{
    ...
    for (i = 0; i < module_inst->function_count; i++) {
        ...
        if (func_inst->is_import_func) {
            func_name = func_inst->u.func_import->field_name->str;
        }
...
#if WASM_ENABLE_DYNAMIC_LINKING != 0
    WASMProgramInstance * program = module_inst->program;
    if (program && program->config.import_memop_mode == FROM_ROOT)
        module_inst = (WASMModuleInstance *)program->root_module_inst;
#endif
...
wasm_set_aux_stack(WASMExecEnv *exec_env, uint32 start_offset, uint32 size)
{
    ...
    if (stack_top_idx != (uint32)-1) {
        ...
        uint8 *global_addr =
            module_inst->globals[stack_top_idx].data;
                                     ...
                                     WASMModuleInstMemConsumption *mem_conspn)
{
    ...
    if (module_inst->global_count > 0) {
        ...
        mem_conspn->globals_size += global->data + wasm_value_type_size(global->type) - (uint8 *)module_inst->globals;
    }
