                                    ...
                                    uint32 func_idx)
{
    ...
    WASMRuntimeFrame *outs_area = wasm_exec_env_wasm_stack_top(exec_env);
    WASMFunctionInstance *cur_func;
    WASMType *func_type;
    uint32 argc, argv_buf[16], *argv = argv_buf, i;
    bool ret = false;

    bh_assert(func_idx >= module_inst->module->import_function_count);

    cur_func = module_inst->e->functions + func_idx;
    func_type = cur_func->u.func->func_type;
    argc = cur_func->param_cell_num;

    /* Allocate memory for arguments if buffer isn't enough */
    if (argc > (uint32)sizeof(argv_buf) / sizeof(uint32)) {
        if (!(argv = runtime_malloc(
                  sizeof(uint32) * argc, module_inst->cur_exception,
                  (uint32)sizeof(module_inst->cur_exception)))) {
            return false;
        }
    }

    /* Prepare arguments */
    bh_memcpy_s(argv, (uint32)sizeof(uint32) * argc, outs_area->lp,
                (uint32)sizeof(uint32) * argc);

    wasm_interp_call_wasm(module_inst, exec_env, cur_func, argc, argv);
    ...
    for (i = 0; i < cur_func->ret_cell_num; i++) {
        *prev_frame->sp++ = argv[i];
    }

    if (func_type->result_count > 0) {
        uint32 *jit_cache = (uint32 *)exec_env->jit_cache;

        /* Copy first result value, the jitted code will read it into
           ret register */
