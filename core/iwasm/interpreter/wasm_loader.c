    ...
    while (flag != WASM_OP_END) {
        switch (flag) {
            ...
            case INIT_EXPR_TYPE_GET_GLOBAL:
            {
                ...
                if (global_idx < module->import_global_count) {
                    global_type =
                        module->import_globals[global_idx].u.global.common.type;
#if WASM_ENABLE_GC != 0
                           ...
                           uint32 error_buf_size)
{
    ...
    if (global->common.type != type
        || global->common.is_mutable != is_mutable) {
        LOG_DEBUG("%s,%s failed type check(%d, %d), expected(%d, %d)",
                   ...
                   uint32 error_buf_size)
{
    ...
    *p_buf = p;
    ...
    ret = wasm_native_lookup_libc_builtin_global(sub_module_name, global_name,
                                                 global);
    if (ret) {
        if (global->common.type != declare_type
            || global->common.is_mutable != declare_mutable) {
            set_error_buf(error_buf, error_buf_size,
                          "incompatible import type");
            return false;
        }
    ...
    global->common.type = declare_type;
    global->common.is_mutable = (declare_mutable == 1);

#if WASM_ENABLE_WAMR_COMPILER != 0
    if (global->common.type == VALUE_TYPE_V128)
        parent_module->is_simd_used = true;
    else if (global->common.type == VALUE_TYPE_EXTERNREF)
        parent_module->is_ref_types_used = true;
#endif
    (void)parent_module;
    (void)ret;
                    ...
                    char *error_buf, uint32 error_buf_size)
{
    ...
    if (global_count) {
        ...
        for (i = 0; i < global_count; i++, global++) {
#if WASM_ENABLE_GC == 0
            CHECK_BUF(p, p_end, 2);
            global->common.type = read_uint8(p);
            mutable = read_uint8(p);
#else
            if (!resolve_value_type(&p, p_end, module, module->type_count,
            ...
            global->common.type = ref_type.ref_type;
            CHECK_BUF(p, p_end, 1);
            mutable = read_uint8(p);
            ...
            if (global->common.type == VALUE_TYPE_V128)
                module->is_simd_used = true;
            else if (global->common.type == VALUE_TYPE_FUNCREF
                     || global->common.type == VALUE_TYPE_EXTERNREF)
                module->is_ref_types_used = true;
#endif
            ...
            global->common.is_mutable = mutable ? true : false;
            ...
            if (global->init_expr.init_expr_type == INIT_EXPR_TYPE_GET_GLOBAL) {
                ...
                if (!wasm_reftype_is_subtype_of(
                        global_type, global_ref_type, global->common.type,
...
#endif
        data_offset += wasm_value_type_size(import_global->common.type);
    }
...
#endif
        data_offset += wasm_value_type_size(global->common.type);
    }
    ...
    for (i = 0; i < module->export_count; i++, export ++) {
        if (export->kind == EXPORT_KIND_GLOBAL) {
            if (!strcmp(export->name, "__heap_base")) {
                ...
                if (global->common.type == VALUE_TYPE_I32
                    && !global->common.is_mutable
            ...
            */
            if (aux_data_end_global && aux_heap_base_global
                ...
                for (global_index = 0; global_index < module->global_count;
                     global_index++) {
                    global = module->globals + global_index;
                    if (global->common.is_mutable /* heap_base and data_end is
                                              not mutable */
                        && global->common.type == VALUE_TYPE_I32
                        && global->init_expr.init_expr_type
                               == INIT_EXPR_TYPE_I32_CONST
    ...
    || ((WASM_ENABLE_WAMR_COMPILER == 0) && (WASM_ENABLE_JIT == 0))
            ...
            case WASM_OP_REF_FUNC:
            {
                ...
                if (func_idx >= module->import_function_count) {
                    ...
                    for (i = 0; i < module->global_count; i++) {
                        if (module->globals[i].common.type == VALUE_TYPE_FUNCREF
