...
extern int _vm_begin_rootset_enumeration (void *heap);

#define GB (1 << 30UL)

#ifdef GC_STAT
void gc_heap_stat(void* heap_ptr, gc_stat_t* stat) {
...
void __gc_print_stat(void *heap_ptr, int verbose) {
	gc_stat_t stat;
	int i;
	...
	printf("# stat %s %x use %d free %d \n", "instance", heap, stat.usage, stat.free);
...
#ifdef STAT_SHOW_GC
void gc_traverse_tree (hmu_tree_node_t *node, gc_size_t* stats, int* n) {
	if (!node)
		return;
...
extern void bh_log_emit(const char *fmt, va_list ap);

static void gc_log_stat (const char *fmt, ...)
{
	va_list ap;
	va_start (ap, fmt);
	bh_log_emit (fmt, ap);
...
void gc_show_fragment (void* heap_arg) {
	int stats[3];
	int n = 3;
	gc_heap_t *heap = (gc_heap_t *)heap_arg;
...
static mark_node_t* alloc_mark_node(void)
{
...
static void free_mark_node(mark_node_t *node)
{
	bh_assert(node);
...
static void wasm_runtime_gc_pre_sweep () {
}

/* @heap should be a valid instance heap which has already been marked*/
static void sweep_instance_heap(gc_heap_t *heap)
{
	hmu_t *cur = NULL, *end = NULL, *last = NULL;
	hmu_type_t ut;
	gc_size_t size;
	...
	/* reset KFC*/
        int lsize = (int)(sizeof(heap->kfc_normal_list) / sizeof(heap->kfc_normal_list[0]));
	for(i = 0;i < lsize; i++)
	{
...
static int add_wo_to_expand(gc_heap_t *heap, gc_object_t obj)
{
	...
	bh_assert(gci_is_heap_valid(heap));
	bh_assert((gc_uint8*)hmu >= heap->base_addr && (gc_uint8*)hmu < heap->base_addr + heap->current_size);
	bh_assert(hmu_get_ut(hmu) == HMU_WO);

	if(hmu_is_wo_marked(hmu)) return GC_SUCCESS;/* already marked*/
	...
	if(!mark_node || mark_node->idx == mark_node->cnt)
	{
		...
		if(!new_node)
		{
			LOG_ERROR("can not add obj to mark node because of mark node allocation failed");
			return GC_ERROR;
		}
...
int gc_add_root(void *heap_p, gc_object_t obj)
{
...
/* @heap should be a valid instance heap*/
static void rollback_mark(gc_heap_t *heap)
{
...
static int reclaim_instance_heap(gc_heap_t *heap)
{
...
#if BH_GC_VERIFY != 0
	/* no matter whether the enumeration is successful or not, the data collected should be checked at first*/
	mark_node = (mark_node_t *)heap->root_set;
	while(mark_node)
	{
		/* all nodes except first should be full filled*/
		bh_assert(mark_node == (mark_node_t*)heap->root_set || mark_node->idx == mark_node->cnt);
	...
	/* 1. mark rootset and organize them into a mark_node list (last marked roots at list header, i.e. stack top)*/
	...
	while(mark_node)
	{
		...
		for(idx = 0;idx < (int)mark_node->idx;idx++)
		{
			...
			if(gct_vm_get_wasm_object_ref_list(obj, &is_compact_mode, &ref_num, &ref_list, &ref_start_offset) == GC_ERROR)
			{
				LOG_ERROR("mark process failed because failed vm_get_wasm_object_ref_list");
				break;
			}
			...
			if(is_compact_mode)
			{
				for(j = 0;j < (int)ref_num;j++)
				{
					...
					if(ref == NULL_REF) continue; /* NULL REF*/
		...
		if(idx < (int)mark_node->idx) break; /* not yet done*/
...
int vm_get_wasm_object_ref_list(
			...
			gc_uint16 **ref_list,
			gc_uint32 *ref_start_offset) {
  os_printf("Error: Unimplemented gct_vm_get_wasm_object_ref_list function!\n");
  return GC_ERROR;
}
