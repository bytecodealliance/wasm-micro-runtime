...
static int unlink_hmu(gc_heap_t *heap, hmu_t *hmu) {
	gc_size_t size;
...
#endif
{
	...
	/* align size*/
	tot_size_unaligned = size + HMU_SIZE + OBJ_PREFIX_SIZE + OBJ_SUFFIX_SIZE; /* hmu header, prefix, suffix*/
...
void gc_free_wo(void* vheap, void* ptr) {
