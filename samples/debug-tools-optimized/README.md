...
`wasm-objdump`, `llvm-cxxfilt`) and the modern-vs-legacy resolver split
that addr2line.py handles internally, see
...
recursive chain. On older wasi-sdks (clang < 22) the function name returned
by the LLVM symbolizer is unreliable on wasm targets; the legacy resolver
path overlays the outermost frame's name from a `.debug_info` scan, which
recovers the correct subprogram name (e.g. `recurse`/`app_main`) regardless
of the symbolizer bug.
