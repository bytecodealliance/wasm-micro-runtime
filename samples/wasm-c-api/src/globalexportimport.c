...
wasm_global_t *
get_export_global(const wasm_extern_vec_t *exports, size_t i)
{
...
wasm_module_t *
create_module_from_file(wasm_store_t *store, const char *filename)
{
...
main(int argc, const char *argv[])
{
    ...
    own wasm_instance_t *instance_import = wasm_instance_new(
        store, moduleimport, NULL,
        NULL); // after this var_f32_export->inst_comm_rt is module_import
    if (!instance_import) {
    ...
    wasm_global_set(
        var_f32_export,
        &val37); //  var_f32_export->inst_comm_rt is module_import now

    check_global(var_f32_export, f32, 37.0);
    check_call(get_var_f32_export, f32,
               37.0); // Call to module export  Failed here, still 7
    ...
    wasm_val_vec_new(
        &args77, 1,
        (wasm_val_t[]){ { .kind = WASM_F32, .of = { .f32 = 77.0 } } });
    ...
    check_call(get_var_f32_import, f32,
               77.0); // Call to module import  Failed here, still 37

    printf("Modify the variable to 78.0...\n");
    wasm_val_vec_t args78;
    wasm_val_vec_new(
        &args78, 1,
        (wasm_val_t[]){ { .kind = WASM_F32, .of = { .f32 = 78.0 } } });
    wasm_func_call(set_var_f32_import, &args78, NULL);
    check_global(var_f32_export, f32, 78.0);
    check_call(get_var_f32_export, f32,
               78.0); // Call to module export Failed here, still 77
