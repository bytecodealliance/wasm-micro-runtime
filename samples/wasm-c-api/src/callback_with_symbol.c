...
int main(int argc, const char* argv[]) {
  ...
  FILE* file = fopen("callback_with_symbol.aot", "rb");
#else
  FILE* file = fopen("callback_with_symbol.wasm", "rb");
#endif
  if (!file) {
  ...
  own wasm_func_t* print_func = wasm_func_new_with_symbol(store, print_type, print_callback,"","print");

  int i = 42;
  own wasm_functype_t* closure_type = wasm_functype_new_0_1(wasm_valtype_new_i32());
  own wasm_func_t* closure_func = wasm_func_new_with_env_symbol(store, closure_type, closure_callback, &i, NULL,"","closure");
  ...
  wasm_extern_t* externs[] = {
    wasm_func_as_extern(closure_func), wasm_func_as_extern(print_func)
  };
