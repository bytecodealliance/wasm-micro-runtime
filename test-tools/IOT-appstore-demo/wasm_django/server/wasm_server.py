...
class Request():
    ...
    def __init__(self, url, action, fmt, payload, payload_len):
        ...
        self.action = action
        self.fmt = fmt
<<<<<<< HEAD:wasm_django/server/wasm_server.py
=======
        
>>>>>>> deb64f5271749c598ed564fa2df9f1af0e19a0d8:wasm_django/devices/wasm_server.py
...
def query(conn):
    ...
    try:
        ...
        while True:
            try:
                byte = conn.recv(1)
                if receive_context.on_imrt_link_byte_arrive(byte) == 0:
                    break
            except socket.error as e:
                pass
...
>>>>>>> deb64f5271749c598ed564fa2df9f1af0e19a0d8:wasm_django/devices/wasm_server.py
    # no payload
    elif len(msg) == 24 and msg[8 + 1] == 65:
...
>>>>>>> deb64f5271749c598ed564fa2df9f1af0e19a0d8:wasm_django/devices/wasm_server.py
    # no payload
    elif len(msg) == 24 and msg[8 + 1] == 66:
            ...
            if s is self.server:
                ...
                if data != b'':
                    ...
                    for dev in self.devices:
                        dev_conn_list.append(dev.conn)

                    # Currently, we only receive command from django
                    if s not in dev_conn_list:
